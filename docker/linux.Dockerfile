...
\
# latest cmake required for vcpkg
export ARCH=$(uname -m) && \
if [ $ARCH == "x86_64" ] || [ $ARCH == "x64" ]; then \
	curl -O https://cmake.org/files/v3.21/cmake-3.21.3-linux-x86_64.sh; \
elif [ $ARCH == "arm64" ] || [ $ARCH == "aarch64" ]; then \
	curl -O https://cmake.org/files/v3.21/cmake-3.21.3-linux-aarch64.sh; \
else \
...
rm -f cmake-*.sh && \
# yay arm64 builds are available so we dont have to self compile
