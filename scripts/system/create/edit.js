...
(function () {
	...
	if (Settings.getValue("experimentalCreateToolsRepositioning")) {
		...
		createToolsWindow.setPosition(
			Window.innerWidth - CREATE_TOOLS_WIDTH,
			0
		);
	...
	// // to enable highlight
	// Selection.addToSelectedItemsList("CurrentlySelectedHighlight", "entity", entityID);
	// // to disabled highlight
	// Selection.removeFromSelectedItemsList("CurrentlySelectedHighlight", "entity", entityID);

	// // to clean everything up on script ending
	// Selection.removeListFromMap("CurrentlySelectedHighlight");
	...
	var shouldUseEditTabletApp = function () {
		return (
			HMD.active ||
			(!HMD.active &&
				!Settings.getValue("desktopTabletBecomesToolbar", true))
		);
	};
	...
	var PARTICLE_SYSTEM_URL = Script.resolvePath(
		"assets/images/icon-particles.svg"
	);
	var POINT_LIGHT_URL = Script.resolvePath(
		"assets/images/icon-point-light.svg"
	);
	var SPOT_LIGHT_URL = Script.resolvePath(
		"assets/images/icon-spot-light.svg"
	);
	var ZONE_URL = Script.resolvePath("assets/images/icon-zone.svg");

	var entityIconOverlayManager = new EntityIconOverlayManager(
		["Light", "ParticleEffect", "Zone"],
		function (entityID) {
	...
	var EntityShapeVisualizer = Script.require(
		"./modules/entityShapeVisualizer.js"
	);
	...
	var MENU_SHOW_LIGHTS_AND_PARTICLES_IN_EDIT_MODE =
		"Show Lights and Particle Systems in Create Mode";
	var MENU_SHOW_ZONES_IN_EDIT_MODE = "Show Zones in Create Mode";

	var MENU_CREATE_ENTITIES_GRABBABLE =
		"Create Entities As Grabbable (except Zones, Particles, and Lights)";
	...
	var SETTING_SHOW_LIGHTS_AND_PARTICLES_IN_EDIT_MODE =
		"showLightsAndParticlesInEditMode";
	...
	var INSUFFICIENT_PERMISSIONS_ERROR_MSG =
		"You do not have the necessary permissions to edit on this domain.";
	var INSUFFICIENT_PERMISSIONS_IMPORT_ERROR_MSG =
		"You do not have the necessary permissions to place items on this domain.";
	...
	var importingSVOTextOverlay = Overlays.addOverlay("text", {
		...
		x:
			Window.innerWidth -
			IMPORTING_SVO_OVERLAY_WIDTH -
			IMPORTING_SVO_OVERLAY_MARGIN,
		y:
			Window.innerHeight -
			IMPORTING_SVO_OVERLAY_HEIGHT -
			IMPORTING_SVO_OVERLAY_MARGIN,
	...
	function adjustPositionPerBoundingBox(
		position,
		direction,
		registration,
		dimensions,
		...
		for (var i = 0, length = CORNERS.length; i < length; i++) {
			var cornerVector = Vec3.multiplyQbyV(
				orientation,
				Vec3.multiplyVbyV(
					Vec3.subtract(CORNERS[i], registration),
					dimensions
				)
			);
	...
	function checkEditPermissionsAndUpdate() {
		...
		var hasRezPermissions =
			Entities.canRez() ||
			Entities.canRezTmp() ||
			Entities.canRezCertified() ||
			Entities.canRezTmpCertified();
	...
	var toolBar = (function () {
		...
		function createNewEntity(requestedProperties) {
			var dimensions = requestedProperties.dimensions
				? requestedProperties.dimensions
				: DEFAULT_DIMENSIONS;
			...
			if (position !== null && position !== undefined) {
				...
				position = grid.snapToSurface(
					grid.snapToGrid(position, false, dimensions),
					dimensions
				);
				...
				if (!properties.grab) {
					properties.grab = {};
					if (
						Menu.isOptionChecked(MENU_CREATE_ENTITIES_GRABBABLE) &&
						!(
				...
				if (POST_ADJUST_ENTITY_TYPES.indexOf(properties.type) !== -1) {
					...
					var initialDimensions = Entities.getEntityProperties(
						entityID,
						["dimensions"]
					).dimensions;
					...
					var dimensionsCheckFunction = function () {
						...
						var properties = Entities.getEntityProperties(
							entityID,
							["dimensions", "registrationPoint", "rotation"]
						);
						if (
							!Vec3.equal(
							...
							dimensionsCheckCount < MAX_DIMENSIONS_CHECKS
						) {
							Script.setTimeout(
								dimensionsCheckFunction,
								DIMENSIONS_CHECK_INTERVAL
							);
						}
					};
					Script.setTimeout(
		...
		function handleNewModelDialogResult(result) {
			if (result) {
				...
				var dynamic =
					result.dynamic !== null ? result.dynamic : DYNAMIC_DEFAULT;
				if (shapeType === "static-mesh" && dynamic) {
					// The prompt should prevent this case
					print(
						"Error: model cannot be both static mesh and dynamic.  This should never happen."
					);
				} else if (url) {
					createNewEntity({
						type: "Model",
		...
		function fromQml(message) {
			// messages are {method, params}, like json-rpc. See also sendToQml.
			var tablet = Tablet.getTablet(
		...
		function initialize() {
			...
			Entities.canRezCertifiedChanged.connect(
				checkEditPermissionsAndUpdate
			);
			Entities.canRezTmpCertifiedChanged.connect(
				checkEditPermissionsAndUpdate
			);
			var hasRezPermissions =
				Entities.canRez() ||
			...
			var createButtonIconRsrc = hasRezPermissions
				? CREATE_ENABLED_ICON
				: CREATE_DISABLED_ICON;
			...
			tablet.screenChanged.connect(function (type, url) {
				var isGoingToHomescreenOnDesktop =
					!shouldUseEditTabletApp() &&
					(url === "hifi/tablet/TabletHome.qml" || url === "");
				if (
					isActive &&
					(type !== "QML" ||
						url !== Script.resolvePath("qml/Edit.qml")) &&
					!isGoingToHomescreenOnDesktop
				) {
					that.setActive(false);
				}
			});
			...
			addButton("importEntitiesButton", function () {
				...
				Window.browseAsync(
					"Select entities json to import",
					"",
					"*.json"
				);
			});
				...
				return function () {
					if (shouldUseEditTabletApp()) {
						...
						tablet.pushOntoStack(
							Script.resolvePath(
								"qml/New" + entityType + "Dialog.qml"
							)
						);
					} else {
						closeExistingDialogWindow();
						var qmlPath = Script.resolvePath(
							"qml/New" + entityType + "Window.qml"
						);
			...
			addButton(
				"newModelButton",
				createNewEntityDialogButtonCallback("Model")
			);
		...
		that.setActive = function (active) {
			...
			if (!isActive) {
				...
				if (shouldUseEditTabletApp()) {
					tablet.loadQMLSource(
						Script.resolvePath("qml/Edit.qml"),
						true
					);
				} else {
			...
			entityIconOverlayManager.setVisible(
				isActive &&
					Menu.isOptionChecked(
						MENU_SHOW_LIGHTS_AND_PARTICLES_IN_EDIT_MODE
					)
			);
			Entities.setDrawZoneBoundaries(
				isActive && Menu.isOptionChecked(MENU_SHOW_ZONES_IN_EDIT_MODE)
			);
		};
	...
	function rayPlaneIntersection2(pickRay, point, normal) {
		...
		if (t < 0.0) {
			...
			return Vec3.sum(
				pickRay.origin,
				Vec3.multiply(pickRay.direction, t)
			);
		}
	}
	...
	function findClickedEntity(event) {
		...
		if (tabletIDs.length > 0) {
			var overlayResult = Overlays.findRayIntersection(
				pickRay,
				true,
				tabletIDs
			);
	...
	function handleOverlaySelectionToolUpdates(channel, message, sender) {
		...
		if (data.method === "selectOverlay") {
			if (
				...
				selectionDisplay.triggeredHand === data.hand
			) {
				if (wantDebug) {
					print("setting selection to overlay " + data.overlayID);
				}
				var entity = entityIconOverlayManager.findEntity(
					data.overlayID
				);
	...
	function mousePressEvent(event) {
		...
		if (
			propertyMenu.mousePressEvent(event) ||
			progressDialog.mousePressEvent(event)
		) {
		...
		if (isActive) {
			if (
				...
				selectionDisplay.mousePressEvent(event)
			) {
	...
	function mouseMove(event) {
		if (mouseDown && !mouseHasMovedSincePress) {
			...
			if (
				timeSincePressMicro < CLICK_TIME_THRESHOLD &&
				sqDist < CLICK_MOVE_DISTANCE_THRESHOLD
			) {
				return;
			}
		...
		if (
			selectionDisplay.mouseMoveEvent(event) ||
			propertyMenu.mouseMoveEvent(event) ||
	...
	function mouseClickEvent(event) {
		...
		if (isActive && event.isLeftButton) {
			...
			var sizeOK =
				(allowLargeModels || angularSize < MAX_ANGULAR_SIZE) &&
			...
			if (0 < x && sizeOK) {
				...
				if (wantDebug) {
					print("Model selected: " + foundEntity); // tivoli is this really
				}
			...
			if (result) {
				...
				// TIVOLI - Turn off right click marketplace inspect
				// Want to do something different here when you RMB?
				// Like a context menu?
				//
				...
				// 	propertyMenu.updateMenuItemText(
				// 		showMenuItem,
				// 		"Show in Marketplace"
				// 	);
				// } else {
	...
	function setupModelMenus() {
		...
		Menu.addMenuItem({
			...
			isCheckable: true,
			isChecked: Settings.getValue(
				SETTING_EDIT_PREFIX + MENU_ALLOW_SELECTION_LARGE,
				true
			)
		});
		Menu.addMenuItem({
			...
			isCheckable: true,
			isChecked: Settings.getValue(
				SETTING_EDIT_PREFIX + MENU_ALLOW_SELECTION_SMALL,
				true
			)
		});
		Menu.addMenuItem({
			...
			isCheckable: true,
			isChecked: Settings.getValue(
				SETTING_EDIT_PREFIX + MENU_ALLOW_SELECTION_LIGHTS,
				false
			)
		});
		Menu.addMenuItem({
		...
		Menu.addMenuItem({
			...
			isCheckable: true,
			isChecked:
				Settings.getValue(SETTING_AUTO_FOCUS_ON_SELECT) === "true"
		});
		Menu.addMenuItem({
			menuName: "Edit",
		...
		Menu.addMenuItem({
			...
			isChecked:
				Settings.getValue(
					SETTING_SHOW_LIGHTS_AND_PARTICLES_IN_EDIT_MODE
				) !== "false"
		});
		Menu.addMenuItem({
			menuName: "Edit",
			...
			isCheckable: true,
			isChecked:
				Settings.getValue(SETTING_SHOW_ZONES_IN_EDIT_MODE) !== "false"
		});
	...
	Script.scriptEnding.connect(function () {
		...
		Settings.setValue(
			SETTING_AUTO_FOCUS_ON_SELECT,
			Menu.isOptionChecked(MENU_AUTO_FOCUS_ON_SELECT)
		);
		Settings.setValue(
			SETTING_EASE_ON_FOCUS,
			Menu.isOptionChecked(MENU_EASE_ON_FOCUS)
		);
		Settings.setValue(
			SETTING_SHOW_LIGHTS_AND_PARTICLES_IN_EDIT_MODE,
			Menu.isOptionChecked(MENU_SHOW_LIGHTS_AND_PARTICLES_IN_EDIT_MODE)
		);
		Settings.setValue(
			SETTING_SHOW_ZONES_IN_EDIT_MODE,
			Menu.isOptionChecked(MENU_SHOW_ZONES_IN_EDIT_MODE)
		);

		Settings.setValue(
			SETTING_EDIT_PREFIX + MENU_ALLOW_SELECTION_LARGE,
			Menu.isOptionChecked(MENU_ALLOW_SELECTION_LARGE)
		);
		Settings.setValue(
			SETTING_EDIT_PREFIX + MENU_ALLOW_SELECTION_SMALL,
			Menu.isOptionChecked(MENU_ALLOW_SELECTION_SMALL)
		);
		Settings.setValue(
			SETTING_EDIT_PREFIX + MENU_ALLOW_SELECTION_LIGHTS,
			Menu.isOptionChecked(MENU_ALLOW_SELECTION_LIGHTS)
		);
	...
	Script.update.connect(function (deltaTime) {
		...
		var dOrientation = Math.abs(
			Quat.dot(Camera.orientation, lastOrientation) - 1
		);
	...
	function insideBox(center, dimensions, point) {
		return (
			Math.abs(point.x - center.x) <= dimensions.x / 2.0 &&
	...
	function selectAllEntitiesInCurrentSelectionBox(keepIfTouching) {
		if (selectionManager.hasSelection()) {
			...
			var boundingBoxCorner = Vec3.subtract(
				selectionManager.worldPosition,
				Vec3.multiply(selectionManager.worldDimensions, 0.5)
			);
			var entities = Entities.findEntitiesInBox(
				boundingBoxCorner,
				selectionManager.worldDimensions
			);
			...
			if (!keepIfTouching) {
				var isValid;
				if (
					selectionManager.localPosition === null ||
					selectionManager.localPosition === undefined
				) {
					...
					isValid = function (position) {
						var localPosition = Vec3.multiplyQbyV(
							Quat.inverse(selectionManager.localRotation),
							Vec3.subtract(
								position,
	...
	function recursiveDelete(
		...
		var initialPropertySets = Entities.getMultipleEntityProperties(
			entities
		);
	...
	function unparentSelectedEntities() {
		if (SelectionManager.hasSelection()) {
			...
			if (selectedEntities.length < 1) {
				Window.notifyEditError(
					"You must have an entity selected in order to unparent it."
				);
				return;
			}
			selectedEntities.forEach(function (id, index) {
				var parentId = Entities.getEntityProperties(id, ["parentID"])
					.parentID;
				if (
					parentId !== null &&
					parentId.length > 0 &&
	...
	function parentSelectedEntities() {
		if (SelectionManager.hasSelection()) {
			...
			if (selectedEntities.length <= 1) {
				Window.notifyEditError(
					"You must have multiple entities selected in order to parent them"
				);
				return;
			}
			var parentCheck = false;
			...
			selectedEntities.forEach(function (id, index) {
				if (lastEntityId !== id) {
					var parentId = Entities.getEntityProperties(id, [
						"parentID"
					]).parentID;
	...
	function deleteSelectedEntities() {
		if (SelectionManager.hasSelection()) {
			...
			SelectionManager.saveProperties();
			var savedProperties = [];
			var newSortedSelection = sortSelectedEntities(
				selectionManager.selections
			);
			var entityHostTypes = Entities.getMultipleEntityProperties(
				newSortedSelection,
			...
			for (var i = 0; i < newSortedSelection.length; ++i) {
				...
				if (
					initialProperties.locked ||
					(initialProperties.avatarEntity &&
						initialProperties.owningAvatarID !==
	...
	function toggleSelectedEntitiesLocked() {
		if (SelectionManager.hasSelection()) {
			var locked = !Entities.getEntityProperties(
	...
	function toggleSelectedEntitiesVisible() {
		if (SelectionManager.hasSelection()) {
			var visible = !Entities.getEntityProperties(
				SelectionManager.selections[0],
				["visible"]
			).visible;
			for (var i = 0; i < selectionManager.selections.length; i++) {
	...
	function moveSelectedEntitiesToAvatar() {
		if (SelectionManager.hasSelection()) {
			for (var i = 0; i < selectionManager.selections.length; i++) {
				var entityID = SelectionManager.selections[i];
				Entities.editEntity(entityID, {
	...
	function handleMenuEvent(menuItem) {
		if (menuItem === "Allow Selecting of Small Models") {
			allowSmallModels = Menu.isOptionChecked(
				"Allow Selecting of Small Models"
			);
		} else if (menuItem === "Allow Selecting of Large Models") {
			allowLargeModels = Menu.isOptionChecked(
				"Allow Selecting of Large Models"
			);
		} else if (menuItem === "Allow Selecting of Lights") {
			Entities.setLightsArePickable(
				Menu.isOptionChecked("Allow Selecting of Lights")
			);
			...
			menuItem === "Import Entities" ||
			menuItem === "Import Entities from URL"
		) {
			...
			Settings.setValue(
				SETTING_EDIT_PREFIX + menuItem,
				Menu.isOptionChecked(menuItem)
			);
		}
		tooltip.show(false);
	}
	...
	function getPositionToCreateEntity(extra) {
		...
		if (
			position.x > HALF_TREE_SCALE ||
			position.y > HALF_TREE_SCALE ||
			position.z > HALF_TREE_SCALE
		) {
			return null;
		}
		return position;
	...
	function importSVO(importURL) {
		...
		if (success) {
			...
			var isLargeImport =
				Clipboard.getClipboardContentsLargestDimension() >= VERY_LARGE;
			var position = Vec3.ZERO;
			if (!isLargeImport) {
				position = getPositionToCreateEntity(
					Clipboard.getClipboardContentsLargestDimension() / 2
				);
			}
			if (position !== null && position !== undefined) {
				...
				if (!isLargeImport) {
					...
					var propType = Entities.getEntityProperties(
						pastedEntityIDs[0],
						["type"]
					).type;
					var NO_ADJUST_ENTITY_TYPES = [
						"Zone",
						"Light",
						"ParticleEffect"
					];
					if (NO_ADJUST_ENTITY_TYPES.indexOf(propType) === -1) {
						...
						targetDirection = Vec3.multiplyQbyV(
							targetDirection,
							Vec3.UNIT_Z
						);
						...
						for (
							var i = 0, length = pastedEntityIDs.length;
							i < length;
							i++
						) {
							var curLoopEntityProps = Entities.getEntityProperties(
								pastedEntityIDs[i],
								[
									"position",
									"dimensions",
							...
							var adjustedPosition = adjustPositionPerBoundingBox(
								targetPosition,
								targetDirection,
								curLoopEntityProps.registrationPoint,
								curLoopEntityProps.dimensions,
								curLoopEntityProps.rotation
							);
							var delta = Vec3.subtract(
								adjustedPosition,
								curLoopEntityProps.position
							);
							var distance = Vec3.dot(delta, targetDirection);
							deltaParallel = Math.min(distance, deltaParallel);
							deltaPerpendicular = Vec3.sum(
								Vec3.subtract(
									delta,
									Vec3.multiply(distance, targetDirection)
								),
								deltaPerpendicular
							);
						...
						deltaPerpendicular = Vec3.multiply(
							1 / pastedEntityIDs.length,
							deltaPerpendicular
						);
						deltaPosition = Vec3.sum(
							Vec3.multiply(deltaParallel, targetDirection),
							deltaPerpendicular
						);
					}

					if (grid.getSnapToGrid()) {
						var firstEntityProps = Entities.getEntityProperties(
							pastedEntityIDs[0],
							["position", "dimensions", "registrationPoint"]
						);
						var positionPreSnap = Vec3.sum(
							deltaPosition,
							firstEntityProps.position
						);
						position = grid.snapToSurface(
							grid.snapToGrid(
								positionPreSnap,
								false,
								firstEntityProps.dimensions,
								firstEntityProps.registrationPoint
							),
							firstEntityProps.dimensions,
							firstEntityProps.registrationPoint
						);
						deltaPosition = Vec3.subtract(
							position,
							firstEntityProps.position
						);
					}
					...
					if (!Vec3.equal(deltaPosition, Vec3.ZERO)) {
						for (
							var editEntityIndex = 0,
								numEntities = pastedEntityIDs.length;
							editEntityIndex < numEntities;
							editEntityIndex++
						) {
							if (Uuid.isNull(entityParentIDs[editEntityIndex])) {
								Entities.editEntity(
									pastedEntityIDs[editEntityIndex],
									{
										position: Vec3.sum(
											deltaPosition,
											entityPositions[editEntityIndex]
										)
									}
								);
							}
				...
				Window.notifyEditError(
					"Can't import entities: entities would be out of bounds."
				);
			}
		} else {
			Window.notifyEditError(
				"There was an error importing the entity file."
			);
		}
	...
	function selectAllKey(value) {
		// CPM Select All
		if (value === 0) {
			var allFoundEntityIDs = Entities.findEntities(
				MyAvatar.position,
				9999999999999
			);
			selectionManager.setSelections(allFoundEntityIDs, this);
		}
	}
	...
	UndoHistory.prototype.pushCommand = function (
		undoFn,
		undoArgs,
		redoFn,
		redoArgs
	...
	var PropertiesTool = function (opts) {
		...
		that.avatarToEntity = function () {
			emitScriptEvent({
				type: "avatarToEntity"
			});
		};

		that.entityToAvatar = function () {
			emitScriptEvent({
				type: "entityToAvatar"
			});
		};
		...
		function updateSelections(selectionUpdated, caller) {
			...
			if (selectionUpdated) {
				...
				if (selectionManager.selections.length !== 1) {
					...
					currentSelectedEntityID !== selectionManager.selections[0]
				) {
					if (statusMonitor !== null) {
						statusMonitor.stop();
					}
					...
					statusMonitor = new ServerScriptStatusMonitor(
						entityID,
						updateScriptStatus
					);
				}
			...
			for (var i = 0; i < selectionManager.selections.length; i++) {
				...
				entity.properties = Entities.getEntityProperties(
					selectionManager.selections[i]
				);
				if (entity.properties.rotation !== undefined) {
					entity.properties.rotation = Quat.safeEulerAngles(
						entity.properties.rotation
					);
				}
				if (entity.properties.localRotation !== undefined) {
					entity.properties.localRotation = Quat.safeEulerAngles(
						entity.properties.localRotation
					);
				}
				if (entity.properties.emitOrientation !== undefined) {
					entity.properties.emitOrientation = Quat.safeEulerAngles(
						entity.properties.emitOrientation
					);
				}
				if (
					entity.properties.keyLight !== undefined &&
					entity.properties.keyLight.direction !== undefined
				) {
					entity.properties.keyLight.direction = Vec3.toPolar(
						entity.properties.keyLight.direction
					);
		...
		var onWebEventReceived = function (data) {
			...
			if (data.type === "update") {
				if (data.properties || data.propertiesMap) {
					...
					propertiesMap.forEach(function (propertiesObject) {
						...
						if (properties.rotation !== undefined) {
							properties.rotation = Quat.fromVec3Degrees(
								properties.rotation
							);
						}
						if (properties.localRotation !== undefined) {
							properties.localRotation = Quat.fromVec3Degrees(
								properties.localRotation
							);
						}
						if (properties.emitOrientation !== undefined) {
							properties.emitOrientation = Quat.fromVec3Degrees(
								properties.emitOrientation
							);
						}
						if (
							properties.keyLight !== undefined &&
							properties.keyLight.direction !== undefined
						) {
							var currentKeyLightDirection = Vec3.toPolar(
							...
							if (properties.keyLight.direction.x === undefined) {
								properties.keyLight.direction.x =
									currentKeyLightDirection.x;
							}
							if (properties.keyLight.direction.y === undefined) {
								properties.keyLight.direction.y =
									currentKeyLightDirection.y;
							}
							properties.keyLight.direction = Vec3.fromPolar(
								properties.keyLight.direction.x,
								properties.keyLight.direction.y
							);
						}
						...
						if (
							...
							properties.modelURL !== undefined ||
							properties.materialURL !== undefined ||
							properties.visible !== undefined ||
							properties.locked !== undefined
						) {
							sendListUpdate = true;
						}
					});
				...
				data.type === "saveUserData" ||
				data.type === "saveMaterialData"
			) {
				...
				if (data.action === "moveSelectionToGrid") {
					if (selectionManager.hasSelection()) {
						...
						dY =
							grid.getOrigin().y -
							(selectionManager.worldPosition.y -
								selectionManager.worldDimensions.y / 2);
						diff = {
							x: 0,
							y: dY,
							z: 0
					...
					if (selectionManager.hasSelection()) {
						...
						for (
							...
							var bottomY =
								properties.boundingBox.center.y -
								properties.boundingBox.dimensions.y / 2;
					...
					if (selectionManager.hasSelection()) {
						...
						for (
							...
							var naturalDimensions =
								properties.naturalDimensions;

							// If any of the natural dimensions are not 0, resize
							if (
								properties.type === "Model" &&
								naturalDimensions.x === 0 &&
								naturalDimensions.y === 0 &&
								naturalDimensions.z === 0
							) {
								Window.notifyEditError(
									"Cannot reset entity to its natural dimensions: Model URL" +
					...
					if (selectionManager.hasSelection()) {
						...
						for (
							...
							Entities.editEntity(entityId, { modelURL: newUrl });
				...
				emitScriptEvent({
					type: "tooltipsReply",
					tooltips: Script.require(
						"./assets/data/createAppTooltips.json"
					),
				...
				data.properties.forEach(function (property) {
					propertyRanges[property] = Entities.getPropertyInfo(
						property
					);
				});
				emitScriptEvent({
				...
				var parentModelData;
				var properties = Entities.getEntityProperties(data.entityID, [
					"type",
					"parentID"
				]);
				if (
					...
					var parentType = Entities.getEntityProperties(
						properties.parentID,
						["type"]
					).type;
					if (
						parentType === "Model" ||
						Entities.getNestableType(properties.parentID) ===
							"avatar"
					) {
						parentModelData = Graphics.getModel(
							properties.parentID
						);
					} else if (
						parentType === "Shape" ||
		...
		if (Settings.getValue("experimentalCreateToolsRepositioning")) {
			createToolsWindow.webEventReceived.connect(onWebEventReceived);
		} else {
	...
	var PopupMenu = function () {
		...
		function cleanup() {
			...
			Entities.canRezCertifiedChanged.disconnect(
				checkEditPermissionsAndUpdate
			);
			Entities.canRezTmpCertifiedChanged.disconnect(
				checkEditPermissionsAndUpdate
			);
		}
	...
	var isOnMacPlatform = Controller.getValue(
		Controller.Hardware.Application.PlatformMac
	);
	...
	//mapping.from([Controller.Hardware.Keyboard.X]).to(toggleKey);
	...
	mapping
		...
		.when([Controller.Hardware.Keyboard.Alt])
		.to(
	...
	mapping
		.from([Controller.Hardware.Keyboard.X])
	...
	mapping.from([Controller.Hardware.Keyboard.X]).to(toggleKey); // CPM X to toggle loca/world space mode to match Unity mapping
	mapping
		.from([Controller.Hardware.Keyboard.C])
		.when([Controller.Hardware.Keyboard.Control])
		.to(
			whenReleased(function () {
	...
	mapping
		.from([Controller.Hardware.Keyboard.V])
		.when([Controller.Hardware.Keyboard.Control])
		.to(
			whenReleased(function () {
	...
	mapping
		.from([Controller.Hardware.Keyboard.D]) // CPM CTRL-SHIFT-D to duplicate
		.when(
			[Controller.Hardware.Keyboard.Control] && [
				Controller.Hardware.Keyboard.Shift
			]
		)
		.to(
			whenReleased(function () {
	...
	mapping
		.from([Controller.Hardware.Keyboard.D]) // CPM Deselect
		.when([Controller.Hardware.Keyboard.Control])
		.to(
			whenReleased(function () {
				deselectKey(0);
	...
	mapping
		.from([Controller.Hardware.Keyboard.A]) // CPM Select all key
		.when([Controller.Hardware.Keyboard.Control])
		.to(
			whenReleased(function () {
				selectAllKey(0);
	...
	mapping
		.from([Controller.Hardware.Keyboard.Z])
		.when([
			Controller.Hardware.Keyboard.Control,
			Controller.Hardware.Keyboard.Shift
		])
	...
	mapping
		.from([Controller.Hardware.Keyboard.P])
		.when([
			Controller.Hardware.Keyboard.Control,
			Controller.Hardware.Keyboard.Shift
		])
	...
	keyUpEventFromUIWindow = function (keyUpEvent) {
		...
		if (
			(!isOnMacPlatform && keyUpEvent.keyCodeString === "Delete") ||
			...
			toggleKey(pressedValue);
			console.log("SWITCH TOGGLER");
		} else if (keyUpEvent.keyCodeString === "F") {
			focusKey(pressedValue);
		} else if (keyUpEvent.keyCodeString === "G") {
			gridKey(pressedValue);
		} else if (keyUpEvent.altKey && keyUpEvent.keyCodeString === "L") {
			toggleSelectedEntitiesLocked();
		} else if (keyUpEvent.controlKey && keyUpEvent.keyCodeString === "A") {
			selectAllKey(0); // SELECT ALL
		} else if (keyUpEvent.controlKey && keyUpEvent.keyCodeString === "D") {
			deselectKey(0); // DESELECT ALL
		} else if (keyUpEvent.controlKey && keyUpEvent.keyCodeString === "X") {
			selectionManager.cutSelectedEntities();
			...
			keyUpEvent.shiftKey &&
			keyUpEvent.controlKey &&
			keyUpEvent.keyCodeString === "D"
		) {
			// DUPLICATE SELECTION
			selectionManager.duplicateSelection();
		} else if (
