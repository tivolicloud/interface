...
var COLOR_ORANGE_HIGHLIGHT = { red: 255, green: 99, blue: 0 };
var COLOR_LIGHTER_HIGHLIGHT = { red: 100, green: 22, blue: 0 };

var editHandleOutlineStyle = {
	outlineUnoccludedColor: COLOR_ORANGE_HIGHLIGHT,
	outlineOccludedColor: COLOR_LIGHTER_HIGHLIGHT,
	fillUnoccludedColor: COLOR_ORANGE_HIGHLIGHT,
	fillOccludedColor: COLOR_LIGHTER_HIGHLIGHT,
	outlineUnoccludedAlpha: 1,
	outlineOccludedAlpha: 1,
...
SelectionManager = (function () {
	...
	function handleEntitySelectionToolUpdates(channel, message, sender) {
		...
		try {
			...
			print(
				"ERROR: entitySelectionTool.handleEntitySelectionToolUpdates - got malformed message"
			);
			return;
		}
		...
		if (messageParsed.method === "selectEntity") {
			if (
				!SelectionDisplay.triggered() ||
				SelectionDisplay.triggeredHand === messageParsed.hand
			) {
				if (wantDebug) {
	...
	that.saveProperties = function () {
		...
		for (var i = 0; i < that.selections.length; i++) {
			...
			that.savedProperties[entityID] = Entities.getEntityProperties(
				entityID
			);
		}
	};
	...
	that.addChildrenEntities = function (
		...
		var entityHostTypes = Entities.getMultipleEntityProperties(
			children,
			"entityHostType"
		);
		for (var i = 0; i < children.length; i++) {
			...
			if (entityHostTypes[i].entityHostType !== entityHostType) {
				if (wantDebug) {
					console.log(
						"Skipping addition of entity " +
							childID +
							" with conflicting entityHostType: " +
	...
	function nonDynamicEntityIsBeingGrabbedByAvatar(properties) {
		...
		for (var i = 0; i < grabJointNames.length; ++i) {
			if (
				avatar.getJointIndex(grabJointNames[i]) ===
				properties.parentJointIndex
			) {
				return true;
			}
		}
	...
	that.duplicateSelection = function () {
		...
		for (var i = 0; i < entitiesToDuplicate.length; i++) {
			...
			if (
				!properties.locked &&
				(!properties.avatarEntity ||
					properties.owningAvatarID === MyAvatar.sessionUUID)
			) {
				...
				for (var j = 0; j < actionIDs.length; ++j) {
					...
					var actionArguments = Entities.getActionArguments(
						properties.id,
						actionID
					);
					if (actionArguments) {
				...
				if (properties.parentID !== Uuid.NULL) {
					duplicatedChildrenWithOldParents[newEntityID] =
						properties.parentID;
				}
		...
		Object.keys(duplicatedChildrenWithOldParents).forEach(function (
			childIDNeedingNewParent
		) {
			var originalParentID =
				duplicatedChildrenWithOldParents[childIDNeedingNewParent];
			var newParentID = originalEntityToNewEntityID[originalParentID];
			if (newParentID) {
				Entities.editEntity(childIDNeedingNewParent, {
	...
	that.createEntities = function (entityProperties) {
		...
		for (var i = 0; i < entityProperties.length; ++i) {
			...
			if (properties.parentID in originalEntityToNewEntityID) {
				properties.parentID =
					originalEntityToNewEntityID[properties.parentID];
			} else {
			...
			if (newEntityID) {
				...
				if (properties.parentID !== Uuid.NULL) {
					createdChildrenWithOldParents[newEntityID] =
						properties.parentID;
				}
	...
	that.copySelectedEntities = function () {
		var entityProperties = Entities.getMultipleEntityProperties(
			that.selections
		);
		var entityHostTypes = Entities.getMultipleEntityProperties(
			that.selections,
	...
	that.pasteEntities = function () {
		...
		var deltaPosition = Vec3.subtract(
			pastePosition,
			entityClipboard.position
		);
	...
	that._update = function (selectionUpdated, caller) {
		...
		if (that.selections.length === 0) {
			...
			for (var i = 1; i < that.selections.length; i++) {
				properties = Entities.getEntityProperties(
					that.selections[i],
					"boundingBox"
				);
		...
		for (var j = 0; j < listeners.length; j++) {
			try {
				listeners[j].callback.call(
					listeners[j].thisContext,
					selectionUpdated === true,
					caller
				);
			} catch (e) {
				print(
					"ERROR: entitySelectionTool.update got exception: " +
						JSON.stringify(e)
				);
			}
		}
...
// SELECTION DISPLAY DEFINITION CONSTANTS

SelectionDisplay = (function () {
	var that = {};
	var color_bounding_edge = { red: 255, green: 255, blue: 255 }; // switching to var for now

	const COLOR_GREEN = { red: 0, green: 255, blue: 0 };
	const COLOR_BLUE = { red: 0, green: 0, blue: 255 };
	const COLOR_RED = { red: 255, green: 0, blue: 0 };
	const COLOR_YELLOW = { red: 255, green: 255, blue: 0 };
	const COLOR_HOVER = { red: 0, green: 0, blue: 0 };
	const COLOR_ROTATE_CURRENT_RING = { red: 255, green: 99, blue: 9 };
	const COLOR_BOUNDING_EDGE_UNLOCKED = { red: 255, green: 255, blue: 255 };
	const COLOR_BOUNDING_EDGE_LOCKED = { red: 255, green: 0, blue: 0 };

	const COLOR_SCALE_CUBE = { red: 5, green: 5, blue: 5 };
	...
	const TRANSLATE_ARROW_CYLINDER_CAMERA_DISTANCE_MULTIPLE = 0.003;
	const TRANSLATE_ARROW_CYLINDER_Y_MULTIPLE = 26; //7.5;
	const TRANSLATE_ARROW_CONE_CAMERA_DISTANCE_MULTIPLE = 0.035;
	const TRANSLATE_ARROW_CONE_OFFSET_CYLINDER_DIMENSION_MULTIPLE = 0.45; //0.83;

	const ROTATE_RING_CAMERA_DISTANCE_MULTIPLE = 0.15;
	const ROTATE_CTRL_SNAP_ANGLE = 22.5;
	const ROTATE_DEFAULT_SNAP_ANGLE = 1;
	const ROTATE_DEFAULT_TICK_MARKS_ANGLE_SOLID = 0.05; // CPM we slam it with many small ones to make the ring appear to be solid
	const ROTATE_DEFAULT_TICK_MARKS_ANGLE = 5; // CPM reasonable amount of tick marks as intended regular
	...
	var handleTranslateXCone = Overlays.addOverlay(
		"shape",
		handlePropertiesTranslateArrowCones
	);
	var handleTranslateXCylinder = Overlays.addOverlay(
		"shape",
		handlePropertiesTranslateArrowCylinders
	);
	Overlays.editOverlay(handleTranslateXCone, {
		userData: EMISSIVE_RED,
		color: COLOR_RED
	});
	...
	var handleTranslateYCone = Overlays.addOverlay(
		"shape",
		handlePropertiesTranslateArrowCones
	);
	var handleTranslateYCylinder = Overlays.addOverlay(
		"shape",
		handlePropertiesTranslateArrowCylinders
	);
	Overlays.editOverlay(handleTranslateYCone, {
		userData: EMISSIVE_GREEN,
		color: COLOR_GREEN
	});
	...
	var handleTranslateZCone = Overlays.addOverlay(
		"shape",
		handlePropertiesTranslateArrowCones
	);
	var handleTranslateZCylinder = Overlays.addOverlay(
		"shape",
		handlePropertiesTranslateArrowCylinders
	);
	Overlays.editOverlay(handleTranslateZCone, {
		userData: EMISSIVE_BLUE,
		color: COLOR_BLUE
	});
	...
	var handlePropertiesRotateRings = {
		...
		outerRadius: ROTATE_RING_IDLE_INNER_RADIUS * 1.5,
		majorTickMarksAngle: ROTATE_DEFAULT_TICK_MARKS_ANGLE_SOLID,
		majorTickMarksLength: -0.08, // CPM How thick will the radius be?
		hasTickMarks: true,
	...
	var handleRotatePitchRing = Overlays.addOverlay(
		"circle3d",
		handlePropertiesRotateRings
	);
	...
	var handleRotateYawRing = Overlays.addOverlay(
		"circle3d",
		handlePropertiesRotateRings
	);
	...
	var handleRotateRollRing = Overlays.addOverlay(
		"circle3d",
		handlePropertiesRotateRings
	);
	...
	var selectionIsLocked;

	var lockedDisplay = Overlays.addOverlay("text3d", {
		text: "locked",
	...
	var handleStretchXCube = Overlays.addOverlay(
		"cube",
		handlePropertiesStretchCubes
	);
	Overlays.editOverlay(handleStretchXCube, {
		userData: EMISSIVE_RED,
		color: COLOR_RED
	});
	var handleStretchYCube = Overlays.addOverlay(
		"cube",
		handlePropertiesStretchCubes
	);
	...
	var handleStretchZCube = Overlays.addOverlay(
		"cube",
		handlePropertiesStretchCubes
	);
	...
	var handleStretchXPanel = Overlays.addOverlay(
		"cube",
		handlePropertiesStretchPanel
	);
	Overlays.editOverlay(handleStretchXPanel, {
		//	userData: EMISSIVE_RED,
		color: COLOR_RED
	});
	var handleStretchYPanel = Overlays.addOverlay(
		"cube",
		handlePropertiesStretchPanel
	);
	...
	var handleStretchZPanel = Overlays.addOverlay(
		"cube",
		handlePropertiesStretchPanel
	);
	...
	overlayNames[lockedDisplay] = "lockedDisplay";
	...
	that.triggerClickMapping = Controller.newMapping(
		Script.resolvePath("") + "-click"
	);
	that.triggerPressMapping = Controller.newMapping(
		Script.resolvePath("") + "-press"
	);
	...
	function pointingAtDesktopWindowOrTablet(hand) {
		var pointingAtDesktopWindow =
			(hand === Controller.Standard.RightHand &&
				SelectionManager.pointingAtDesktopWindowRight) ||
			(hand === Controller.Standard.LeftHand &&
				SelectionManager.pointingAtDesktopWindowLeft);
		var pointingAtTablet =
			(hand === Controller.Standard.RightHand &&
				SelectionManager.pointingAtTabletRight) ||
			(hand === Controller.Standard.LeftHand &&
				SelectionManager.pointingAtTabletLeft);
	...
	function makeClickHandler(hand) {
		return function (clicked) {
			...
			if (
				!that.triggered() &&
				clicked &&
				!pointingAtDesktopWindowOrTablet(hand)
			) {
	...
	that.triggerClickMapping
		.from(Controller.Standard.RTClick)
		.peek()
		.to(makeClickHandler(Controller.Standard.RightHand));
	that.triggerClickMapping
		.from(Controller.Standard.LTClick)
		.peek()
		.to(makeClickHandler(Controller.Standard.LeftHand));
	that.triggerPressMapping
		.from(Controller.Standard.RT)
		.peek()
		.to(makePressHandler(Controller.Standard.RightHand));
	that.triggerPressMapping
		.from(Controller.Standard.LT)
		.peek()
		.to(makePressHandler(Controller.Standard.LeftHand));
	...
	function testRayIntersect(queryRay, overlayIncludes, overlayExcludes) {
		...
		if (queryRay === undefined || queryRay === null) {
			if (wantDebug) {
				print(
					"testRayIntersect - EARLY EXIT -> queryRay is undefined OR null!"
				);
			}
			return null;
		}
		...
		var intersectObj = Overlays.findRayIntersection(
			queryRay,
			true,
			overlayIncludesLayered,
			overlayExcludes
		);

		if (!intersectObj.intersects && overlayIncludesNonLayered.length > 0) {
			intersectObj = Overlays.findRayIntersection(
				queryRay,
				true,
				overlayIncludesNonLayered,
				overlayExcludes
			);
		}
		...
		if (wantDebug) {
			...
			print(
				"    intersectObj.overlayID:" +
					intersectObj.overlayID +
					"[" +
					overlayNames[intersectObj.overlayID] +
					"]"
			);
			print(
				"        OverlayName: " + overlayNames[intersectObj.overlayID]
			);
	...
	function isPointInsideBox(point, box) {
		...
		return (
			Math.abs(position.x) <= box.dimensions.x / 2 &&
			Math.abs(position.y) <= box.dimensions.y / 2 &&
	...
	that.resetPreviousHandleColor = function () {
		const shaderPath = this.getShaderPath(previousHandleColor, false);
		//console.log(shaderPath);
		if (previousHandle !== null) {
			if (previousHandle == handleScaleCube) {
			...
			Overlays.editOverlay(previousHandle, {
				...
				majorTickMarksColor: previousHandleColor
			});
			previousHandle = null;
		}
		if (previousHandleHelper !== null) {
			Entities.editEntity(previousHandleHelper, {
				userData: ""
			});
			Entities.editEntity(previousHandleHelper, {
				userData: shaderPath
			});
			Overlays.editOverlay(previousHandleHelper, {
				color: previousHandleColor,
				majorTickMarksColor: previousHandleColor
			});
	...
	that.updateHighlight = function (event) {
		//console.log("HIGHLIGHT CHECKING");
		...
		if (result.intersects) {
			...
			if (highlightNeeded) {
				//console.log("HIGHLIGHT NEEDED FOR ", result.overlayID);
				that.resetPreviousHandleColor();
				//  Overlays.editOverlay(result.overlayID, { color: COLOR_HOVER });
				// console.log(
				// 	"HIGH LIGHT NEEDED PICKED COLOR WAS ",
				// 	JSON.stringify(pickedColor)
				// );

				const shaderPath = this.getShaderPath(pickedColor, true);

				const step1 = JSON.parse(shaderPath);
				const step2 = unescape(JSON.stringify(shaderPath));
				// "{\"ProceduralEntity\":{\"shaderUrl\":\"file:///C:/Program Files/Tivoli Cloud/resources/app/interface/0.86.0/scripts/system/create/assets/shaders/yellowEmissive.fs\",\"version\":3}}"

				//console.log("HERE IS THE THING " + step2);
				...
				Overlays.editOverlay(result.overlayID, {
					color: COLOR_YELLOW,
					majorTickMarksColor: COLOR_YELLOW
				});

				if (result.overlayID == handleScaleCube) {
					Overlays.editOverlay(result.overlayID, {
						solid: true
					});
				}
	...
	that.getShaderPath = function (someColor, highlight) {
		if (someColor == undefined) return EMISSIVE_YELLOW;

		// This is really hacky. Just checking the "color"
		// assigned by the old system and using it to figure out the corresponding
		// colored shader.   5 is just a magic number to say indicate the global scale cube.

		if (highlight) {
			return EMISSIVE_YELLOW;
			if (someColor.red == 255) return EMISSIVE_RED_HIGHLIGHT;
			if (someColor.green == 255) return EMISSIVE_GREEN_HIGHLIGHT;
			if (someColor.blue == 255) return EMISSIVE_BLUE_HIGHLIGHT;
			if (someColor.red == 5) return EMISSIVE_YELLOW_HIGHLIGHT;
		}
		if (someColor.red == 255) return EMISSIVE_RED;
		if (someColor.green == 255) return EMISSIVE_GREEN;
		if (someColor.blue == 255) return EMISSIVE_BLUE;
		if (someColor.red == 5) return EMISSIVE_YELLOW;
	...
	that.mouseMoveEvent = function (event) {
		...
		if (activeTool) {
			if (wantDebug) {
				print(
					"    Trigger ActiveTool(" + activeTool.mode + ")'s onMove"
				);
			}
			...
			if (activeTool.onEnd) {
				...
				Messages.sendLocalMessage(
					...
					JSON.stringify({
						method: "editing",
						hand:
							that.editingHand === Controller.Standard.LeftHand
								? LEFT_HAND
								: RIGHT_HAND,
						editing: false
					})
				...
				print(
					"    ActiveTool(" + activeTool.mode + ")'s missing onEnd"
				);
			}
		}
	...
	that.checkControllerMove = function () {
		if (SelectionManager.hasSelection()) {
			...
			var hand =
				that.triggeredHand === Controller.Standard.LeftHand ? 0 : 1;
			if (
				controllerPose.valid &&
				lastControllerPoses[hand].valid &&
				that.triggered()
			) {
				if (
					!Vec3.equal(
						controllerPose.position,
						lastControllerPoses[hand].position
					) ||
					!Vec3.equal(
						controllerPose.rotation,
						lastControllerPoses[hand].rotation
					)
				) {
					that.mouseMoveEvent({});
				}
	...
	function getControllerAvatarFramePositionFromPickRay(pickRay) {
		var controllerPosition = Vec3.subtract(
			pickRay.origin,
			MyAvatar.position
		);
		controllerPosition = Vec3.multiplyQbyV(
			Quat.inverse(MyAvatar.orientation),
			controllerPosition
		);
	...
	function getDistanceToCamera(position) {
		...
		var toCameraDistance = Vec3.length(
			Vec3.subtract(cameraPosition, position)
		);
		return toCameraDistance;
	}

	function usePreviousPickRay(
		pickRayDirection,
		previousPickRayDirection,
		normal
	) {
		return (
			(Vec3.dot(pickRayDirection, normal) > 0 &&
				Vec3.dot(previousPickRayDirection, normal) < 0) ||
			(Vec3.dot(pickRayDirection, normal) < 0 &&
				Vec3.dot(previousPickRayDirection, normal) > 0)
		);
	}
	...
	that.setSpaceMode = function (newSpaceMode, isDesiredChange) {
		...
		if (spaceMode !== newSpaceMode) {
			...
			print(
				"WARNING: entitySelectionTool.setSpaceMode - Can't update SpaceMode. CurrentMode: " +
					spaceMode +
					" DesiredMode: " +
					newSpaceMode
			);
	...
	that.toggleSpaceMode = function () {
		...
		if (
			...
			if (wantDebug) {
				print(
					"Local space editing is not available with multiple selections"
				);
			}
			return;
		}
		...
		that.setSpaceMode(
			spaceMode === SPACE_LOCAL ? SPACE_WORLD : SPACE_LOCAL,
			true
		);
		if (wantDebug) {
	...
	function isActiveTool(toolHandle) {
		...
		if (!handleTools.hasOwnProperty(toolHandle)) {
			print(
				"WARNING: entitySelectionTool.isActiveTool - Encountered unknown grabberToolHandle: " +
					toolHandle +
					". Tools should be registered via addHandleTool."
			);
			// EARLY EXIT
			return false;
		}
	...
	that.updateHandles = function () {
		...
		if (wantDebug) {
			...
			print(
				"    Selections Count: " + SelectionManager.selections.length
			);
		...
		if (SelectionManager.hasSelection()) {
			var checkForLock = SelectionManager.selections;
			var isLocked = SelectionManager.selections.some(function (
				entityID
			) {
				return Entities.getEntityProperties(entityID, ["locked"])
					.locked;
			});

			color_bounding_edge = isLocked
				? COLOR_BOUNDING_EDGE_LOCKED
				: COLOR_BOUNDING_EDGE_UNLOCKED;

			var position = SelectionManager.worldPosition;
			var rotation =
				spaceMode === SPACE_LOCAL
					? SelectionManager.localRotation
					: SelectionManager.worldRotation;
			var dimensions =
				spaceMode === SPACE_LOCAL
					? SelectionManager.localDimensions
					: SelectionManager.worldDimensions;
			var rotationInverse = Quat.inverse(rotation);
			var toCameraDistance = getDistanceToCamera(position);

			var rotationDegrees = 90;
			var localRotationX = Quat.fromPitchYawRollDegrees(
				0,
				0,
				-rotationDegrees
			);
			...
			var localRotationY = Quat.fromPitchYawRollDegrees(
				0,
				rotationDegrees,
				0
			);
			...
			var localRotationZ = Quat.fromPitchYawRollDegrees(
				rotationDegrees,
				0,
				0
			);
			...
			var isCameraInsideBox = isPointInsideBox(
				Camera.position,
			...
			var rotateDimension = Math.max(
				maxHandleDimension,
				toCameraDistance * ROTATE_RING_CAMERA_DISTANCE_MULTIPLE
			);
			...
			// UPDATE TRANSLATION ARROWS CPM
			var arrowCylinderDimension =
				(rotateDimension *
					TRANSLATE_ARROW_CYLINDER_CAMERA_DISTANCE_MULTIPLE) /
				ROTATE_RING_CAMERA_DISTANCE_MULTIPLE;
			var arrowCylinderDimensions = {
				x: arrowCylinderDimension * 0.65,
				y: arrowCylinderDimension * TRANSLATE_ARROW_CYLINDER_Y_MULTIPLE,
				z: arrowCylinderDimension * 0.65
			};
			var arrowConeDimension =
				(rotateDimension *
					TRANSLATE_ARROW_CONE_CAMERA_DISTANCE_MULTIPLE) /
				ROTATE_RING_CAMERA_DISTANCE_MULTIPLE;
			var arrowConeDimensions = {
				x: arrowConeDimension * 0.45,
				y: arrowConeDimension * 0.95,
				z: arrowConeDimension * 0.45
			};
			var arrowCylinderOffset =
				(rotateDimension * TRANSLATE_ARROW_CYLINDER_OFFSET) /
				ROTATE_RING_CAMERA_DISTANCE_MULTIPLE;
			var arrowConeOffset =
				arrowCylinderDimensions.y *
				TRANSLATE_ARROW_CONE_OFFSET_CYLINDER_DIMENSION_MULTIPLE;
			var cylinderXPosition = { x: arrowCylinderOffset, y: 0, z: 0 };
			cylinderXPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, cylinderXPosition)
			);
			Overlays.editOverlay(handleTranslateXCylinder, {
			...
			var coneXPosition = Vec3.sum(
				cylinderXPosition,
				Vec3.multiply(Vec3.normalize(cylinderXOffset), arrowConeOffset)
			);
			Overlays.editOverlay(handleTranslateXCone, {
			...
			cylinderYPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, cylinderYPosition)
			);
			Overlays.editOverlay(handleTranslateYCylinder, {
			...
			var coneYPosition = Vec3.sum(
				cylinderYPosition,
				Vec3.multiply(Vec3.normalize(cylinderYOffset), arrowConeOffset)
			);
			Overlays.editOverlay(handleTranslateYCone, {
			...
			cylinderZPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, cylinderZPosition)
			);
			Overlays.editOverlay(handleTranslateZCylinder, {
			...
			var coneZPosition = Vec3.sum(
				cylinderZPosition,
				Vec3.multiply(Vec3.normalize(cylinderZOffset), arrowConeOffset)
			);
			Overlays.editOverlay(handleTranslateZCone, {
			...
			// UPDATE SCALE CUBE CPM (global scale cube)
			var scaleCubeRotation =
				spaceMode === SPACE_LOCAL ? rotation : Quat.IDENTITY;
			var scaleCubeDimension =
				(rotateDimension * SCALE_OVERLAY_CAMERA_DISTANCE_MULTIPLE) /
				ROTATE_RING_CAMERA_DISTANCE_MULTIPLE;
			var scaleCubeDimensions = {
				x: scaleCubeDimension,
				y: scaleCubeDimension,
				z: scaleCubeDimension
			};
			Overlays.editOverlay(handleScaleCube, {
				position: position,
				rotation: scaleCubeRotation,
				dimensions: scaleCubeDimensions
				//	userData: EMISSIVE_YELLOW_HIGHLIGHT
				//solid: false
			});
			...
			RBFPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, RBFPosition)
			);
			var RTFPosition = {
				x: edgeOffsetX,
				y: edgeOffsetY,
			...
			RTFPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, RTFPosition)
			);
			var LTNPosition = {
				x: -edgeOffsetX,
				y: edgeOffsetY,
			...
			LTNPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, LTNPosition)
			);
			var RTNPosition = {
				x: edgeOffsetX,
				y: edgeOffsetY,
			...
			RTNPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, RTNPosition)
			);

			var RBFPositionRotated = Vec3.multiplyQbyV(
				rotationInverse,
				RBFPosition
			);
			var RTFPositionRotated = Vec3.multiplyQbyV(
				rotationInverse,
				RTFPosition
			);
			var LTNPositionRotated = Vec3.multiplyQbyV(
				rotationInverse,
				LTNPosition
			);
			var RTNPositionRotated = Vec3.multiplyQbyV(
				rotationInverse,
				RTNPosition
			);
			var stretchPanelXDimensions = Vec3.subtract(
				RTNPositionRotated,
				RBFPositionRotated
			);
			...
			var stretchPanelXPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, { x: dimensions.x / 2, y: 0, z: 0 })
			);
			Overlays.editOverlay(handleStretchXPanel, {
			...
			var stretchPanelYDimensions = Vec3.subtract(
				LTNPositionRotated,
				RTFPositionRotated
			);
			...
			var stretchPanelYPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, { x: 0, y: dimensions.y / 2, z: 0 })
			);
			Overlays.editOverlay(handleStretchYPanel, {
			...
			var stretchPanelZDimensions = Vec3.subtract(
				LTNPositionRotated,
				RBFPositionRotated
			);
			...
			var stretchPanelZPosition = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, { x: 0, y: 0, z: dimensions.z / 2 })
			);
			Overlays.editOverlay(handleStretchZPanel, {
			...
			// UPDATE STRETCH CUBES CPM
			var stretchCubeDimension =
				(rotateDimension * STRETCH_CUBE_CAMERA_DISTANCE_MULTIPLE) /
				ROTATE_RING_CAMERA_DISTANCE_MULTIPLE;
			var stretchCubeDimensions = {
				// CPM
				x: stretchCubeDimension * 1.25,
				y: stretchCubeDimension * 0.5,
				z: stretchCubeDimension * 1.25
			};
			var stretchCubeOffset =
				(rotateDimension * STRETCH_CUBE_OFFSET) /
				ROTATE_RING_CAMERA_DISTANCE_MULTIPLE;
			var stretchXPosition, stretchYPosition, stretchZPosition;
			if (isActiveTool(handleStretchXCube)) {
				stretchXPosition = Vec3.subtract(
					stretchPanelXPosition,
					activeStretchCubePanelOffset
				);
			} else {
				stretchXPosition = { x: stretchCubeOffset, y: 0, z: 0 };
				stretchXPosition = Vec3.sum(
					position,
					Vec3.multiplyQbyV(rotation, stretchXPosition)
				);
			}
			if (isActiveTool(handleStretchYCube)) {
				stretchYPosition = Vec3.subtract(
					stretchPanelYPosition,
					activeStretchCubePanelOffset
				);
			} else {
				stretchYPosition = { x: 0, y: stretchCubeOffset, z: 0 };
				stretchYPosition = Vec3.sum(
					position,
					Vec3.multiplyQbyV(rotation, stretchYPosition)
				);
			}
			if (isActiveTool(handleStretchZCube)) {
				stretchZPosition = Vec3.subtract(
					stretchPanelZPosition,
					activeStretchCubePanelOffset
				);
			} else {
				stretchZPosition = { x: 0, y: 0, z: stretchCubeOffset };
				stretchZPosition = Vec3.sum(
					position,
					Vec3.multiplyQbyV(rotation, stretchZPosition)
				);
			}
			Overlays.editOverlay(handleStretchXCube, {
			...
			var inModeRotate =
				isActiveTool(handleRotatePitchRing) ||
			...
			if (
				...
				iconSelectionBoxGeometry.visible =
					!inModeRotate && isCameraInsideBox;
				iconSelectionBoxGeometry.ignorePickIntersection = !iconSelectionBoxGeometry.visible;
				Overlays.editOverlay(
					iconSelectionBox,
					iconSelectionBoxGeometry
				);
			} else {
			...
			var handleDuplicatorPos = Vec3.sum(
				position,
				Vec3.multiplyQbyV(rotation, handleDuplicatorOffset)
			);
		...
		that.setHandleTranslateXVisible(
			!activeTool ||
				isActiveTool(handleTranslateXCone) ||
				isActiveTool(handleTranslateXCylinder)
		);
		that.setHandleTranslateYVisible(
			!activeTool ||
				isActiveTool(handleTranslateYCone) ||
				isActiveTool(handleTranslateYCylinder)
		);
		that.setHandleTranslateZVisible(
			!activeTool ||
				isActiveTool(handleTranslateZCone) ||
				isActiveTool(handleTranslateZCylinder)
		);
		that.setHandleRotatePitchVisible(
			!activeTool || isActiveTool(handleRotatePitchRing)
		);
		that.setHandleRotateYawVisible(
			!activeTool || isActiveTool(handleRotateYawRing)
		);
		that.setHandleRotateRollVisible(
			!activeTool || isActiveTool(handleRotateRollRing)
		);

		var showScaleStretch =
			!activeTool &&
			SelectionManager.selections.length === 1 &&
			spaceMode === SPACE_LOCAL;
		that.setHandleStretchXVisible(
			showScaleStretch || isActiveTool(handleStretchXCube)
		);
		that.setHandleStretchYVisible(
			showScaleStretch || isActiveTool(handleStretchYCube)
		);
		that.setHandleStretchZVisible(
			showScaleStretch || isActiveTool(handleStretchZCube)
		);
		that.setHandleScaleVisible(
			showScaleStretch || isActiveTool(handleScaleCube)
		);

		var showOutlineForZone =
			SelectionManager.selections.length === 1 &&
			typeof SelectionManager.savedProperties[
				SelectionManager.selections[0]
			] !== "undefined" &&
			SelectionManager.savedProperties[SelectionManager.selections[0]]
				.type === "Zone";
		that.setHandleBoundingBoxVisible(
			showOutlineForZone ||
				(!isActiveTool(handleRotatePitchRing) &&
					!isActiveTool(handleRotateYawRing) &&
					!isActiveTool(handleRotateRollRing))
	...
	that.updateActiveRotateRing = function () {
		...
		if (activeRotateRing !== null) {
			var tickMarksAngle = ctrlPressed
				? ROTATE_CTRL_SNAP_ANGLE
				: ROTATE_DEFAULT_TICK_MARKS_ANGLE;
			Overlays.editOverlay(activeRotateRing, {
				majorTickMarksAngle: tickMarksAngle
			});
		}
	};
	...
	that.setOverlaysVisible = function (isVisible) {
		for (var i = 0, length = allOverlays.length; i < length; i++) {
			Overlays.editOverlay(allOverlays[i], {
				visible: isVisible,
				ignorePickIntersection: !isVisible
			});
		}
	};
	...
	that.showDebugPickPlaneHit = function (pickHitPosition) {
		var dimension =
			getDistanceToCamera(pickHitPosition) *
			DEBUG_PICK_PLANE_HIT_CAMERA_DISTANCE_MULTIPLE;
	...
	function addHandleTranslateXZTool(overlay, mode, doDuplicate) {
		...
		addHandleTool(overlay, {
			...
			onBegin: function (event, pickRay, pickResult) {
				...
				greatestDimension = Math.max(
					Math.max(
						SelectionManager.worldDimensions.x,
						SelectionManager.worldDimensions.y
					),
					SelectionManager.worldDimensions.z
				);
				startingDistance = Vec3.distance(
					pickRay.origin,
					SelectionManager.position
				);
				startingElevation = this.elevation(
					pickRay.origin,
					pickPlanePosition
				);
				if (wantDebug) {
				...
				initialPick = rayPlaneIntersection(
					pickRay,
					pickPlanePosition,
					pickPlaneNormal
				);
			...
			elevation: function (origin, intersection) {
				return (
					...
					Vec3.distance(origin, intersection)
				);
			},
			onMove: function (event) {
				...
				var newPick = rayPlaneIntersection2(
					pickRay,
					pickPlanePosition,
					pickPlaneNormal
				);
				...
				if (!newPick) {
					if (wantDebug) {
						print(
							"    " +
								mode +
								"Pick ray does not intersect XZ plane."
						);
					}
				...
				if (greatestDimension > 0) {
					var angularSize = Math.atan(
						greatestDimension /
							Vec3.distance(pickRay.origin, newPick)
					);
					if (wantDebug) {
				...
				vector = Vec3.subtract(
					grid.snapToGrid(
						Vec3.sum(cornerPosition, vector),
						constrainMajorOnly
					),
					cornerPosition
				);
				...
				var toMove = SelectionManager.selections.filter(function (
					selection
				) {
					if (
						SelectionManager.selections.indexOf(
							SelectionManager.savedProperties[selection].parentID
						) >= 0
					) {
				...
				for (var i = 0; i < toMove.length; i++) {
					var properties =
						SelectionManager.savedProperties[toMove[i]];
					if (!properties) {
						continue;
					}
	...
	function addHandleTranslateTool(overlay, mode, direction) {
		...
		addHandleTool(overlay, {
			...
			onBegin: function (event, pickRay, pickResult) {
				...
				pickPlaneNormal = Vec3.cross(
					Vec3.cross(pickRay.direction, axisVector),
					axisVector
				);
				...
				that.setHandleTranslateXVisible(
					direction === TRANSLATE_DIRECTION.X
				);
				that.setHandleTranslateYVisible(
					direction === TRANSLATE_DIRECTION.Y
				);
				that.setHandleTranslateZVisible(
					direction === TRANSLATE_DIRECTION.Z
				);
				that.setHandleRotateVisible(false);
				that.setHandleStretchVisible(false);
				that.setHandleScaleVisible(false);
			...
			onMove: function (event) {
				...
				var newPick = rayPlaneIntersection(
					pickRay,
					pickPlanePosition,
					pickPlaneNormal
				);
				...
				projectionVector = Vec3.multiplyQbyV(
					rotation,
					projectionVector
				);
				...
				vector = Vec3.subtract(
					grid.snapToGrid(Vec3.sum(vector, gridOrigin)),
					gridOrigin
				);
	...
	function addHandleStretchTool(overlay, mode, directionEnum) {
		...
		addHandleTool(overlay, {
			...
			onBegin: function (event, pickRay, pickResult) {
				...
				var scaledOffsetWorld = Vec3.multiplyVbyV(
					initialDimensions,
					offsetRP
				);
				...
				pickPlanePosition = Vec3.sum(
					initialPosition,
					Vec3.multiplyQbyV(rotation, scaledOffsetWorld)
				);
				initialPick = rayPlaneIntersection(
					pickRay,
					pickPlanePosition,
				...
				that.setHandleStretchXVisible(
					directionEnum === STRETCH_DIRECTION.X
				);
				that.setHandleStretchYVisible(
					directionEnum === STRETCH_DIRECTION.Y
				);
				that.setHandleStretchZVisible(
					directionEnum === STRETCH_DIRECTION.Z
				);
				that.setHandleDuplicatorVisible(false);
				...
				if (properties.collidesWith.indexOf(collisionToRemove) > -1) {
					var newCollidesWith = properties.collidesWith.replace(
						collisionToRemove,
						""
					);
					Entities.editEntity(SelectionManager.selections[0], {
						collidesWith: newCollidesWith
					});
					that.replaceCollisionsAfterStretch = true;
				}

				if (stretchPanel !== null) {
					Overlays.editOverlay(stretchPanel, {
						visible: true,
						ignorePickIntersection: false
					});
				}
				var stretchCubePosition = Overlays.getProperty(
					handleStretchCube,
					"position"
				);
				var stretchPanelPosition = Overlays.getProperty(
					stretchPanel,
					"position"
				);
				activeStretchCubePanelOffset = Vec3.subtract(
					stretchPanelPosition,
					stretchCubePosition
				);
			...
			onEnd: function (event, reason) {
				if (that.replaceCollisionsAfterStretch) {
					var newCollidesWith =
						SelectionManager.savedProperties[
							SelectionManager.selections[0]
						].collidesWith;
					Entities.editEntity(SelectionManager.selections[0], {
						collidesWith: newCollidesWith
			...
			onMove: function (event) {
				...
				changeInDimensions = Vec3.multiplyQbyV(
					Quat.inverse(rotation),
					changeInDimensions
				);
				changeInDimensions = Vec3.multiplyVbyV(
					mask,
					changeInDimensions
				);
				changeInDimensions = grid.snapToSpacing(changeInDimensions);
				changeInDimensions = Vec3.multiply(
					NEGATE_VECTOR,
					Vec3.multiplyVbyV(signs, changeInDimensions)
				);

				var newDimensions = Vec3.sum(
					initialDimensions,
					changeInDimensions
				);

				var minimumDimension = Entities.getPropertyInfo("dimensions")
					.minimum;
				if (newDimensions.x < minimumDimension) {
					newDimensions.x = minimumDimension;
					changeInDimensions.x =
						minimumDimension - initialDimensions.x;
				}
				if (newDimensions.y < minimumDimension) {
					newDimensions.y = minimumDimension;
					changeInDimensions.y =
						minimumDimension - initialDimensions.y;
				}
				if (newDimensions.z < minimumDimension) {
					newDimensions.z = minimumDimension;
					changeInDimensions.z =
						minimumDimension - initialDimensions.z;
				}

				var changeInPosition = Vec3.multiplyQbyV(
					rotation,
					Vec3.multiplyVbyV(deltaPivot, changeInDimensions)
				);
	...
	function addHandleScaleTool(overlay, mode) {
		...
		addHandleTool(overlay, {
			...
			onMove: function (event) {
				...
				var dimensionsMultiple =
					toCameraDistance *
					SCALE_DIMENSIONS_CAMERA_DISTANCE_MULTIPLE;
				var changeInDimensions = Vec3.subtract(newPick, initialPick);
				changeInDimensions = Vec3.multiplyQbyV(
					Quat.inverse(Camera.orientation),
					changeInDimensions
				);
				changeInDimensions = grid.snapToSpacing(changeInDimensions);
				changeInDimensions = Vec3.multiply(
				...
				var averageDimensionChange =
					(changeInDimensions.x +
						changeInDimensions.y +
						changeInDimensions.z) /
					3;
				var averageInitialDimension =
					(initialDimensions.x +
						initialDimensions.y +
						initialDimensions.z) /
					3;
				percentChange =
					averageDimensionChange / averageInitialDimension;
				percentChange += 1.0;
	...
	function updateRotationDegreesOverlay(angleFromZero, position) {
		...
		var overlayProps = {
			...
			lineHeight:
				toCameraDistance * ROTATE_DISPLAY_LINE_HEIGHT_MULTIPLIER,
			text: normalizeDegrees(-angleFromZero) + "°"
		};
		Overlays.editOverlay(rotationDegreesDisplay, overlayProps);
	}

	// function updateLockedDisplayOverlay(isLocked, position) {
	// 	var toCameraDistance = getDistanceToCamera(position);
	// 	var overlayProps = {
	// 		position: { position },
	// 		dimensions: {
	// 			x: 100, //toCameraDistance * ROTATE_DISPLAY_SIZE_X_MULTIPLIER,
	// 			y: 100 //toCameraDistance * ROTATE_DISPLAY_SIZE_Y_MULTIPLIER
	// 		},
	// 		lineHeight: 20,
	// 		//	toCameraDistance * ROTATE_DISPLAY_LINE_HEIGHT_MULTIPLIER,
	...
	// 		visible: isLocked
	// 	};
	// 	Overlays.editOverlay(lockedDisplay, overlayProps);
	// }
	...
	function updateSelectionsRotation(rotationChange, initialPosition) {
		if (!rotationChange) {
			print(
				"ERROR: entitySelectionTool.updateSelectionsRotation - Invalid arg specified!!"
			);
		...
		for (var i = 0; i < toRotate.length; i++) {
			...
			var newProperties = {
				rotation: Quat.multiply(
					rotationChange,
					initialProperties.rotation
				)
			};

			if (reposition) {
				var dPos = Vec3.subtract(
					initialProperties.position,
					initialPosition
				);
	...
	function addHandleRotateTool(overlay, mode, direction) {
		...
		addHandleTool(overlay, {
			...
			onBegin: function (event, pickRay, pickResult) {
				...
				initialRotation =
					spaceMode === SPACE_LOCAL
						? SelectionManager.localRotation
						: SelectionManager.worldRotation;
				rotationNormal = Vec3.multiplyQbyV(
					initialRotation,
					rotationNormal
				);
				...
				that.setHandleTranslateVisible(false);
				that.setHandleRotatePitchVisible(
					direction === ROTATE_DIRECTION.PITCH
				);
				that.setHandleRotateYawVisible(
					direction === ROTATE_DIRECTION.YAW
				);
				that.setHandleRotateRollVisible(
					direction === ROTATE_DIRECTION.ROLL
				);
				that.setHandleStretchVisible(false);
				that.setHandleScaleVisible(false);
				that.setHandleDuplicatorVisible(false);
				...
				var initialPick = rayPlaneIntersection(
					pickRay,
					rotationCenter,
					rotationNormal
				);
				...
				var rotationCenterToZero = Vec3.subtract(
					rotationZero,
					rotationCenter
				);
				var rotationCenterToZeroLength = Vec3.length(
					rotationCenterToZero
				);
				rotationDegreesPosition = Vec3.sum(
					rotationCenter,
					Vec3.multiply(
						Vec3.normalize(rotationCenterToZero),
						rotationCenterToZeroLength *
							ROTATE_DISPLAY_DISTANCE_MULTIPLIER
					)
				);
			...
			onMove: function (event) {
				if (!rotationZero) {
					print(
						"ERROR: entitySelectionTool.addHandleRotateTool.onMove - " +
				...
				if (wantDebug) {
					print(
						"================== " +
							getMode() +
							"(addHandleRotateTool onMove) -> ======================="
					);
					Vec3.print("    rotationZero: ", rotationZero);
				}

				var pickRay = generalComputePickRay(event.x, event.y);
				var result = rayPlaneIntersection(
					pickRay,
					rotationCenter,
					rotationNormal
				);
				if (result) {
					var centerToZero = Vec3.subtract(
						rotationZero,
						rotationCenter
					);
					var centerToIntersect = Vec3.subtract(
						result,
						rotationCenter
					);
					...
					var angleFromZero = Vec3.orientedAngle(
						centerToZero,
						centerToIntersect,
						rotationNormal
					);
					var snapAngle = ctrlPressed
						? ROTATE_CTRL_SNAP_ANGLE
						: ROTATE_DEFAULT_SNAP_ANGLE;
					angleFromZero =
						Math.floor(angleFromZero / snapAngle) * snapAngle;
					var rotationChange = Quat.angleAxis(
						angleFromZero,
						rotationNormal
					);
	...
	addHandleTranslateTool(
		handleTranslateXCone,
		"TRANSLATE_X",
		TRANSLATE_DIRECTION.X
	);
	addHandleTranslateTool(
		handleTranslateXCylinder,
		"TRANSLATE_X",
	...
	addHandleRotateTool(
		handleRotatePitchRing,
		"ROTATE_PITCH",
		ROTATE_DIRECTION.PITCH
	);
	addHandleRotateTool(
		handleRotateYawRing,
		"ROTATE_YAW",
		ROTATE_DIRECTION.YAW
	);
	addHandleRotateTool(
		handleRotateRollRing,
		"ROTATE_ROLL",
		ROTATE_DIRECTION.ROLL
	);
