...
const PROFILE = !ENABLE_PROFILING
	? PROFILE_NOOP
	: function (name, fn, args) {
			console.log(
				"PROFILE-Web " + profileIndent + "(" + name + ") Begin"
			);
...
function loaded() {
	openEventBridge(function () {
		...
		elFilterTypeMultiselectBox = document.getElementById(
			"filter-type-multiselect-box"
		);
		...
		elFilterTypeOptionsButtons = document.getElementById(
			"filter-type-options-buttons"
		);
		elFilterTypeSelectAll = document.getElementById(
		...
		elSelectedEntitiesCount = document.getElementById(
			"selected-entities-count"
		);
		elVisibleEntitiesCount = document.getElementById(
			"visible-entities-count"
		);
		elNoEntitiesMessage = document.getElementById("no-entities");
		elColumnsMultiselectBox = document.getElementById(
			"entity-table-columns-multiselect-box"
		);
		elColumnsOptions = document.getElementById(
			"entity-table-columns-options"
		);
		elToggleSpaceMode = document.getElementById("toggle-space-mode");
		elAvatarToEntity = document.getElementById("avatar-to-entity");
		elEntityToAvatar = document.getElementById("entity-to-avatar");
		...
		elToggleSpaceMode.onclick = function () {
			EventBridge.emitWebEvent(
				JSON.stringify({ type: "toggleSpaceMode" })
			);
		};
		elAvatarToEntity.onclick = function () {
			print("Avatar to entity");
			EventBridge.emitWebEvent(
				JSON.stringify({ type: "avatarToEntity" })
			);
		};
		elEntityToAvatar.onclick = function () {
			EventBridge.emitWebEvent(
				JSON.stringify({ type: "entityToAvatar" })
			);
		};
		...
		for (let columnID in COLUMNS) {
			...
			elTh.onmousedown = function (event) {
				if (event.target.nodeName === "TH") {
					...
					targetColumnIndex = parseInt(
						elTargetTh.getAttribute("columnIndex")
					);
			...
			let initialWidth =
				columnData.initiallyShown === true
					? columnData.initialWidth
					: 0;
			columns.push({
				columnID: columnID,
				elTh: elTh,
		...
		elEntityTableHeaderRow = document.querySelectorAll(
		...
		entityListContextMenu.setOnSelectedCallback(function (
			optionName,
			selectedEntityID
		) {
			switch (optionName) {
		...
		function onRowClicked(clickEvent) {
			...
			if (controlKey) {
				...
				if (previousItemFound !== -1 && clickedItemFound !== -1) {
					...
					for (
						let i = Math.min(previousItemFound, clickedItemFound);
						i <= toItem;
						i++
					) {
				...
				if (selectedEntities[0] === entityID) {
					if (
						renameLastBlur &&
						renameLastEntityID === entityID &&
						Date.now() - renameLastBlur < RENAME_COOLDOWN
					) {
						return;
					}
					clearRenameTimeout();
		...
		function refreshEntityList() {
			PROFILE("refresh-entity-list", function () {
				...
				PROFILE("sort", function () {
					...
					let isDefaultSort =
						currentSortOrder ===
						COLUMNS[currentSortColumnID].defaultSortOrder;
		...
		function clearEntities() {
			...
			for (
				...
				i <= lastVisibleRow && i < visibleEntities.length;
				i++
			) {
				let entity = visibleEntities[i];
		...
		function updateRow(elRow, itemData) {
			...
			let validPrevItemID =
				prevEntityID !== undefined && prevEntityID !== EMPTY_ENTITY_ID;
			if (
				validPrevItemID &&
				prevEntityID !== newEntityID &&
				entitiesByID[prevEntityID].elRow === elRow
			) {
				elRow.dataset.entityID = EMPTY_ENTITY_ID;
		...
		function onToggleFilterInView() {
			...
			if (isFilterInView) {
				elFilterInView.setAttribute(
					FILTER_IN_VIEW_ATTRIBUTE,
					FILTER_IN_VIEW_ATTRIBUTE
				);
			} else {
		...
		function onRadiusChange() {
			...
			EventBridge.emitWebEvent(
				JSON.stringify({ type: "radius", radius: elFilterRadius.value })
			);
			refreshEntities();
		}
		...
		function toggleColumnsDropdown() {
			elColumnsOptions.style.display = isColumnsDropdownVisible()
				? "none"
				: "block";
		}
			...
			if (columnChecked) {
				...
				for (let i = 0; i < columns.length; ++i) {
					...
					if (column.columnID !== columnID && column.width > 0) {
						...
						column.width +=
							(column.width / newTotalWidth) * widthLoss;
					}
				}
			}
		...
		function toggleTypeDropdown() {
			elFilterTypeOptions.style.display = isTypeDropdownVisible()
				? "none"
				: "block";
		}
		...
		function onStartResize(event) {
			...
			resizeColumnIndex = parseInt(
				this.parentNode.getAttribute("columnIndex")
			);
			event.stopPropagation();
		}
		...
		function updateColumnWidths() {
			...
			let scrollbarVisible =
				elEntityTableScroll.scrollHeight >
				elEntityTableScroll.clientHeight;
			let resizerRight = scrollbarVisible
				? SCROLLBAR_WIDTH - RESIZER_WIDTH / 2
				: -RESIZER_WIDTH / 2;
			...
			for (let i = columns.length - 1; i > 0; --i) {
				...
				column.elTh.style.visibility = columnVisible
					? "visible"
					: "hidden";
				if (column.elResizer) {
					column.elResizer.style = "right:" + resizerRight + "px;";
					column.elResizer.style.visibility =
						columnVisible && visibleColumns > 0
							? "visible"
							: "hidden";
				}
			...
			if (column.elResizer) {
				column.elResizer.style = "right:" + resizerRight + "px;";
				column.elResizer.style.visibility =
					columnVisible && visibleColumns > 0 ? "visible" : "hidden";
			}
		...
		document.onmousemove = function (event) {
			if (lastResizeEvent) {
				...
				if (
					newColWidth * fullWidth >= MINIMUM_COLUMN_WIDTH &&
					newNextColWidth * fullWidth >= MINIMUM_COLUMN_WIDTH
				) {
					column.width += dPct;
				...
				if (targetColumnIndex < columns.length - 1) {
					...
					let nextColumnStartX = nextColumnTh.getBoundingClientRect()
						.left;
					if (
						event.clientX >= nextColumnStartX &&
						event.clientX - lastColumnSwapPosition >=
							DELTA_X_COLUMN_SWAP_POSITION
					) {
				...
				if (targetColumnIndex >= 1) {
					...
					let prevColumnEndX = prevColumnTh.getBoundingClientRect()
						.right;
					if (
						event.clientX <= prevColumnEndX &&
						lastColumnSwapPosition - event.clientX >=
							DELTA_X_COLUMN_SWAP_POSITION
					) {
		...
		function setSpaceMode(spaceMode) {
			if (spaceMode === "local") {
				elToggleSpaceMode.className =
					"space-mode-local hifi-edit-button";
				elToggleSpaceMode.innerText = "Local";
			} else {
				elToggleSpaceMode.className =
					"space-mode-world hifi-edit-button";
		...
		document.addEventListener(
			...
			function (keyUpEvent) {
				...
				if (controlKey && keyCodeString === "A") {
					let visibleEntityIDs = visibleEntities.map(
						visibleEntity => visibleEntity.id
					);
					let selectionIncludesAllVisibleEntityIDs = visibleEntityIDs.every(
						visibleEntityID => {
		...
		if (window.EventBridge !== undefined) {
			EventBridge.scriptEventReceived.connect(function (data) {
				...
				if (data.type === "clearEntityList") {
					...
					let notFound = updateSelectedEntities(
						data.selectedIDs,
						true
					);
					if (notFound) {
					...
					data.type === "removeEntities" &&
					data.deletedIDs !== undefined &&
					data.selectedIDs !== undefined
				) {
