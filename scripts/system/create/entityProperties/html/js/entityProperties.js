...
const GROUPS = [
	{
		...
		properties: [
			...
			},
			{
				...
				multiDisplayMode:
					PROPERTY_MULTI_DISPLAY_MODE.COMMA_SEPARATED_VALUES
			},
			{
				label: "Description",
				type: "string",
		...
		properties: [
			...
			},
			{
				...
				buttons: [
					{
						id: "copy",
						label: "Copy from Skybox",
						className: "black",
						onClick: copySkyboxURLToAmbientURL
					}
				],
		...
		properties: [
			...
			},
			{
				...
				buttons: [
					{
						id: "clear",
						label: "Clear Material Data",
						className: "red",
						onClick: clearMaterialData
					},
					{
						id: "edit",
						label: "Edit as JSON",
						className: "blue",
						onClick: newJSONMaterialEditor
					},
					{
						id: "save",
						label: "Save Material Data",
						className: "black",
			...
			},
			{
				...
				selectionVisibility:
					PROPERTY_SELECTION_VISIBILITY.SINGLE_SELECTION
			},
			{
				label: "Priority",
				type: "number-draggable",
				...
				buttons: [
					{
						...
						label: "Rescale",
						className: "blue",
						onClick: rescaleDimensions
					},
					{
						id: "reset",
						label: "Reset Dimensions",
						className: "red",
						onClick: resetToNaturalDimensions
					}
				],
				propertyID: "scale"
			},
...
const ENABLE_DISABLE_SELECTOR =
	"input, textarea, span, .dropdown dl, .color-picker";
	...
	switch (property.data.type) {
		...
		case "vec2":
			return {
				x: property.elNumberX.elInput,
				y: property.elNumberY.elInput,
				z: property.elNumberZ.elInput
			};
		case "color":
			return {
				red: property.elNumberR.elInput,
				green: property.elNumberG.elInput,
				blue: property.elNumberB.elInput
			};
		case "icon":
...
function enableChildren(el, selector) {
	...
	for (
		let selectorIndex = 0;
		selectorIndex < elSelectors.length;
		++selectorIndex
	) {
...
function resetProperties() {
	for (let propertyID in properties) {
		...
		switch (propertyData.type) {
			...
			case "color": {
				property.elColorPicker.style.backgroundColor =
					"rgb(" + 0 + "," + 0 + "," + 0 + ")";
...
function resetServerScriptStatus() {
	let elServerScriptError = document.getElementById(
		"property-serverScripts-error"
	);
	let elServerScriptStatus = document.getElementById(
...
function getGroupsForTypes(types) {
	return Object.keys(elGroups).filter(groupKey => {
		return types
			.map(type => GROUPS_PER_TYPE[type].includes(groupKey))
			.every(function (hasGroup) {
				return hasGroup;
			});
	});
}
...
function isCurrentlyDraggingProperty(propertyName) {
	return (
		properties[propertyName] && properties[propertyName].dragging === true
	);
}

const SUPPORTED_FALLBACK_TYPES = [
	"number",
	"number-draggable",
	"rect",
	"vec3",
...
function getMultiplePropertyValue(originalPropertyName) {
	...
	if (splitPropertyName.length > 1) {
		...
		propertyValues = currentSelections.map(selection => {
			...
			if (
				...
				groupProperties[propertyName] === undefined
			) {
				return undefined;
			}
			if (
				splitPropertyName.length ===
				PROPERTY_NAME_DIVISION.SUB_PROPERTY + 1
			) {
				let subPropertyName =
					splitPropertyName[PROPERTY_NAME_DIVISION.SUB_PROPERTY];
		...
		propertyValues = currentSelections.map(
			selection => selection.properties[originalPropertyName]
		);
	}

	if (
		propertyData !== null &&
		propertyData.fallbackProperty !== undefined &&
		...
		for (let i = 0; i < propertyValues.length; ++i) {
			...
			switch (propertyData.type) {
				...
				case "number-draggable":
					isPropertyNotNumber =
						isNaN(propertyValue) || propertyValue === null;
					break;
				case "rect":
				case "vec3":
				case "vec2":
					isPropertyNotNumber =
						isNaN(propertyValue.x) || propertyValue.x === null;
					break;
				case "color":
					isPropertyNotNumber =
						isNaN(propertyValue.red) || propertyValue.red === null;
					break;
			}
			if (isPropertyNotNumber) {
				if (fallbackMultiValue === null) {
					fallbackMultiValue = getMultiplePropertyValue(
	...
	const isMultiDiffValue = !propertyValues.every(x =>
		deepEqual(firstValue, x)
	);
...
function getDetailedNumberMPVDiff(multiplePropertyValue, propertyData) {
	...
	multiplePropertyValue.values.forEach(function (propertyValue) {
		if (typeof propertyValue === "object") {
			Object.entries(propertyValue).forEach(function ([key, value]) {
				...
				detailedValues[key].push(
					applyInputNumberPropertyModifiers(value, propertyData)
				);
			...
			detailedValues[DEFAULT_KEY].push(
				applyInputNumberPropertyModifiers(propertyValue, propertyData)
			);
	...
	Object.entries(unmodifiedValues).forEach(function ([key, value]) {
		...
		averagePerPropertyComponent[key] = applyInputNumberPropertyModifiers(
			average,
			propertyData
		);
	});
...
function getDetailedSubPropertyMPVDiff(multiplePropertyValue, subPropertyName) {
	let isChecked = false;
	let checkedValues = multiplePropertyValue.values.map(value =>
		value.split(",").includes(subPropertyName)
	);
...
function updateVisibleSpaceModeProperties() {
	for (let propertyID in properties) {
		if (properties.hasOwnProperty(propertyID)) {
			...
			if (
				propertySpaceMode !== PROPERTY_SPACE_MODE.ALL &&
				propertySpaceMode !== currentSpaceMode
			) {
...
function updateProperty(
	...
	let propertyUpdate = createPropertyUpdateObject(
		originalPropertyName,
		propertyValue
	);
	...
	if (isParticleProperty) {
		Object.keys(propertyUpdate).forEach(function (propertyUpdateKey) {
			particlePropertyUpdates[propertyUpdateKey] =
				propertyUpdate[propertyUpdateKey];
		});
		...
		let onlyUpdateEntity = isCurrentlyDraggingProperty(
			originalPropertyName
		);
...
function createEmitCheckedPropertyUpdateFunction(property) {
	return function () {
		updateProperty(
			property.name,
			property.data.inverse ? !this.checked : this.checked,
	...
	return function () {
		...
		if (this.multiDiffModeEnabled) {
			...
			for (let i = 0; i < selectedEntityIDsArray.length; ++i) {
				...
				updateObjects.push({
					entityIDs: [entityID],
					properties: createPropertyUpdateObject(
						property.name,
						propertyMultiValue.values[i]
					)
				});
			}
...
function createEmitNumberPropertyUpdateFunction(property) {
	return function () {
		let value = parseFloat(
			applyOutputNumberPropertyModifiers(
...
function createEmitNumberPropertyComponentUpdateFunction(
	property,
	propertyComponent
) {
	return function () {
		...
		if (propertyMultiValue.isMultiDiffValue) {
			...
			for (let i = 0; i < selectedEntityIDsArray.length; ++i) {
				...
				let updateObject = createPropertyUpdateObject(
					property.name,
					propertyObject
				);
				updateObjects.push({
...
function createEmitColorPropertyUpdateFunction(property) {
	return function () {
		emitColorPropertyUpdate(
			property.name,
			property.elNumberR.elInput.value,
			property.elNumberG.elInput.value,
			property.elNumberB.elInput.value,
			property.isParticleProperty
		);
	};
}

function emitColorPropertyUpdate(
	propertyName,
	red,
	green,
	blue,
...
function updateCheckedSubProperty(
	propertyName,
	propertyMultiValue,
	subPropertyElement,
	subPropertyString,
	...
	if (propertyMultiValue.isMultiDiffValue) {
		...
		for (let i = 0; i < selectedEntityIDsArray.length; ++i) {
			let newValue = toggleBooleanCSV(
				propertyMultiValue.values[i],
				subPropertyString,
				subPropertyElement.checked
			);
			updateObjects.push({
           ...
           <input id="${elementID}"
                  ...
                  ${
						propertyData.placeholder
							? 'placeholder="' + propertyData.placeholder + '"'
							: ""
					}
...
function createBoolProperty(property, elProperty) {
	...
	elProperty.appendChild(elInput);
	elProperty.appendChild(
		createElementFromHTML(`<label for=${elementID}>&nbsp;</label>`)
	);
	...
	if (subPropertyOf !== undefined) {
		...
		elInput.addEventListener(
			"change",
			createEmitCheckedPropertyUpdateFunction(property)
		);
	}
...
function createNumberProperty(property, elProperty) {
	...
	elInput.addEventListener(
		"change",
		createEmitNumberPropertyUpdateFunction(property)
	);
...
function createMultiDiffStepFunction(property, propertyComponent) {
	return function (step, shouldAddToUndoHistory) {
		...
		if (!propertyMultiValue.isMultiDiffValue) {
			console.log(
				"setMultiDiffStepFunction is only supposed to be called in MultiDiff mode."
			);
			return;
		}

		let multiplier =
			property.data.multiplier !== undefined
				? property.data.multiplier
				: 1;
		...
		if (selectedEntityIDs.size !== propertyMultiValue.values.length) {
			console.log(
				"selectedEntityIDs and propertyMultiValue got out of sync."
			);
			return;
		}
		let updateObjects = [];
		...
		for (let i = 0; i < selectedEntityIDsArray.length; ++i) {
			...
			let propertiesUpdate = createPropertyUpdateObject(
				property.name,
				updatedValue
			);
			updateObjects.push({
...
function createNumberDraggableProperty(property, elProperty) {
	...
	let elDraggableNumber = new DraggableNumber(
		propertyData.min,
		propertyData.max,
		propertyData.step,
		propertyData.decimals,
		dragStartFunction,
		dragEndFunction
	);
	...
	elDraggableNumber.setMultiDiffStepFunction(
		createMultiDiffStepFunction(property)
	);
	...
	if (propertyData.buttons !== undefined) {
		addButtons(
			elDraggableNumber.elDiv,
			elementID,
			propertyData.buttons,
			false
		);
	}
...
function createRectProperty(property, elProperty) {
	...
	let elNumberX = createTupleNumberInput(
		property,
		propertyData.subLabels[RECT_ELEMENTS.X_NUMBER]
	);
	let elNumberY = createTupleNumberInput(
		property,
		propertyData.subLabels[RECT_ELEMENTS.Y_NUMBER]
	);
	let elNumberWidth = createTupleNumberInput(
		property,
		propertyData.subLabels[RECT_ELEMENTS.WIDTH_NUMBER]
	);
	let elNumberHeight = createTupleNumberInput(
		property,
		propertyData.subLabels[RECT_ELEMENTS.HEIGHT_NUMBER]
	);
	...
	elNumberX.setValueChangeFunction(
		createEmitNumberPropertyComponentUpdateFunction(property, "x")
	);
	elNumberY.setValueChangeFunction(
		createEmitNumberPropertyComponentUpdateFunction(property, "y")
	);
	elNumberWidth.setValueChangeFunction(
		createEmitNumberPropertyComponentUpdateFunction(property, "width")
	);
	elNumberHeight.setValueChangeFunction(
		createEmitNumberPropertyComponentUpdateFunction(property, "height")
	);

	elNumberX.setMultiDiffStepFunction(
		createMultiDiffStepFunction(property, "x")
	);
	elNumberY.setMultiDiffStepFunction(
		createMultiDiffStepFunction(property, "y")
	);
	elNumberX.setMultiDiffStepFunction(
		createMultiDiffStepFunction(property, "width")
	);
	elNumberY.setMultiDiffStepFunction(
		createMultiDiffStepFunction(property, "height")
	);
...
function createVec3Property(property, elProperty) {
	...
	let elNumberX = createTupleNumberInput(
		property,
		propertyData.subLabels[VECTOR_ELEMENTS.X_NUMBER]
	);
	let elNumberY = createTupleNumberInput(
		property,
		propertyData.subLabels[VECTOR_ELEMENTS.Y_NUMBER]
	);
	let elNumberZ = createTupleNumberInput(
		property,
		propertyData.subLabels[VECTOR_ELEMENTS.Z_NUMBER]
	);
	...
	elNumberZ.setValueChangeFunction(
		createEmitNumberPropertyComponentUpdateFunction(property, "z")
	);
	...
	elNumberZ.setMultiDiffStepFunction(
		createMultiDiffStepFunction(property, "z")
	);
...
function createDynamicMultiselectProperty(property, elProperty) {
	...
	elDivButtons.setAttribute(
		"id",
		elDivOptions.getAttribute("id") + "-buttons"
	);
	...
	let buttons = [
		{
			...
			label: "Select All",
			className: "black",
			onClick: selectAllMaterialTarget
		},
		{
			id: "clearAll",
			label: "Clear All",
			className: "black",
			onClick: clearAllMaterialTarget
		}
	];
...
function createProperty(
	...
	switch (propertyType) {
		...
		case "number-draggable": {
			property.elNumber = createNumberDraggableProperty(
				property,
				elProperty
			);
			break;
		...
		case "dropdown": {
			property.elInput = createDropdownProperty(
				property,
				propertyID,
				elProperty
			);
		...
		case "dynamic-multiselect": {
			property.elDivOptions = createDynamicMultiselectProperty(
				property,
				elProperty
			);
			break;
		...
		default: {
			console.log(
				"EntityProperties - Unknown property type " +
					propertyType +
					" set to property " +
					propertyID
			);
...
function parentIDChanged() {
	if (
		...
		currentSelections[0].properties.type === "Material"
	) {
	...
	EventBridge.emitWebEvent(
		JSON.stringify({
			...
			percentage: parseFloat(
				document.getElementById("property-scale").value
			)
		})
	);
}
...
function reloadServerScripts() {
	...
	document.getElementById(
		"property-serverScripts-status"
	).innerText = PENDING_SCRIPT_STATUS;
	EventBridge.emitWebEvent(
		JSON.stringify({
			type: "action",
...
function setJSONError(property, isError) {
	...
	let $propertyUserDataEditorStatus = $(
		"#property-" + property + "-editorStatus"
	);
	$propertyUserDataEditorStatus.css("display", isError ? "block" : "none");
	$propertyUserDataEditorStatus.text(
		isError ? "Invalid JSON code - look for red X in your code" : ""
	);
}
...
function showStaticUserData() {
	if (editor !== null) {
		...
		$propertyUserDataStatic.css(
			"height",
			$("#property-userData-editor").height()
		);
...
function saveJSONUserData(noUpdate, entityIDsToUpdate) {
	setUserDataFromEditor(
		noUpdate,
		entityIDsToUpdate ? entityIDsToUpdate : selectedEntityIDs
	);
...
function showStaticMaterialData() {
	if (materialEditor !== null) {
		...
		$propertyMaterialDataStatic.css(
			"height",
			$("#property-materialData-editor").height()
		);
...
function bindAllNonJSONEditorElements() {
	...
	for (i = 0; i < inputs.length; ++i) {
		...
		field.on("focus", function (e) {
			if (
				e.target.id === "property-userData-button-edit" ||
				e.target.id === "property-userData-button-clear" ||
				e.target.id === "property-materialData-button-edit" ||
				e.target.id === "property-materialData-button-clear"
			) {
				return;
			}
...
function addMaterialTarget(elDivOptions, targetID, isMaterialName) {
	...
	let elLabel = document.createElement("label");
	elLabel.setAttribute("for", elementID);
	elLabel.innerText = isMaterialName
		? "Material " + targetID
		: "Mesh Index " + targetID;
...
function applyInputNumberPropertyModifiers(number, propertyData) {
	const multiplier =
		propertyData.multiplier !== undefined ? propertyData.multiplier : 1;
...
function handleEntitySelectionUpdate(selections, isPropertiesToolUpdate) {
	...
	const hasSelectedEntityChanged = !areSetsEqual(
		selectedEntityIDs,
	...
	if (selections.length === 0) {
		...
		if (
			!isPropertiesToolUpdate &&
			!hasSelectedEntityChanged &&
			document.hasFocus()
		) {
		...
		const entityTypes = [
			...new Set(
				currentSelections.map(a =>
					shapeTypes.includes(a.properties.type)
						? "Shape"
						: a.properties.type
				)
			)
		...
		const certificateIDMultiValue = getMultiplePropertyValue(
			"certificateID"
		);
		const hasCertifiedInSelection =
			certificateIDMultiValue.isMultiDiffValue ||
			certificateIDMultiValue.value !== "";
		...
		Object.entries(properties).forEach(function ([propertyID, property]) {
			...
			if (propertyData.selectionVisibility !== undefined) {
				...
				if (!multipleSelections) {
					...
					propertyVisible = isFlagSet(
						visibility,
						PROPERTY_SELECTION_VISIBILITY.MULTI_DIFF_SELECTIONS
					);
				} else {
					propertyVisible = isFlagSet(
						visibility,
						PROPERTY_SELECTION_VISIBILITY.MULTIPLE_SELECTIONS
					);
				}
			...
			switch (propertyData.type) {
				...
				case "bool": {
					const inverse =
						propertyData.inverse !== undefined
							? propertyData.inverse
							: false;
					if (isSubProperty) {
						let subPropertyMultiValue = getMultiplePropertyValue(
							propertyData.subPropertyOf
						);
						let propertyValue = subPropertyMultiValue.value;
						isMultiDiffValue =
							subPropertyMultiValue.isMultiDiffValue;
						if (isMultiDiffValue) {
							let detailedSubProperty = getDetailedSubPropertyMPVDiff(
								subPropertyMultiValue,
								propertyName
							);
							property.elInput.checked =
								detailedSubProperty.isChecked;
							property.elInput.classList.toggle(
								"multi-diff",
								detailedSubProperty.isMultiDiff
							);
						} else {
							let subProperties = propertyValue.split(",");
							let subPropertyValue =
								subProperties.indexOf(propertyName) > -1;
							property.elInput.checked = inverse
								? !subPropertyValue
								: subPropertyValue;
							property.elInput.classList.remove("multi-diff");
						}
				...
				case "number": {
					property.elInput.value = isMultiDiffValue
						? ""
						: propertyValue;
					property.elInput.classList.toggle(
						"multi-diff",
				...
				case "number-draggable": {
					let detailedNumberDiff = getDetailedNumberMPVDiff(
						propertyMultiValue,
						propertyData
					);
					property.elNumber.setValue(
						detailedNumberDiff.averagePerPropertyComponent[0],
						detailedNumberDiff.propertyComponentDiff[0]
					);
					break;
				}
				case "rect": {
					...
					property.elNumberX.setValue(
						detailedNumberDiff.averagePerPropertyComponent.x,
						detailedNumberDiff.propertyComponentDiff.x
					);
					property.elNumberY.setValue(
						detailedNumberDiff.averagePerPropertyComponent.y,
						detailedNumberDiff.propertyComponentDiff.y
					);
					property.elNumberWidth.setValue(
						detailedNumberDiff.averagePerPropertyComponent.width,
						detailedNumberDiff.propertyComponentDiff.width
					);
					property.elNumberHeight.setValue(
						detailedNumberDiff.averagePerPropertyComponent.height,
						detailedNumberDiff.propertyComponentDiff.height
					);
					break;
				}
				case "vec3":
				case "vec2": {
					...
					if (property.elNumberZ !== undefined) {
						property.elNumberZ.setValue(
							detailedNumberDiff.averagePerPropertyComponent.z,
							detailedNumberDiff.propertyComponentDiff.z
						);
					}
					break;
				}
				case "color": {
					let displayColor = propertyMultiValue.isMultiDiffValue
						? propertyMultiValue.values[0]
						: propertyValue;
					property.elColorPicker.style.backgroundColor =
						"rgb(" +
						...
						displayColor.blue +
						")";
					property.elColorPicker.classList.toggle(
						"multi-diff",
						propertyMultiValue.isMultiDiffValue
					);
				...
				case "icon": {
					property.elSpan.innerHTML =
						propertyData.icons[propertyValue];
		...
		if (json !== null) {
			...
			// normal text
			deleteJSONEditor();
			userDataTextArea.classList.toggle(
				"multi-diff",
				userDataMultiValue.isMultiDiffValue
			);
			userDataTextArea.value = userDataMultiValue.isMultiDiffValue
				? ""
				: userDataMultiValue.value;
		...
		if (materialJson !== null) {
			...
			// normal text
			deleteJSONMaterialEditor();
			materialDataTextArea.classList.toggle(
				"multi-diff",
				materialDataMultiValue.isMultiDiffValue
			);
			materialDataTextArea.value = materialDataMultiValue.isMultiDiffValue
				? ""
				: materialDataMultiValue.value;
			showMaterialDataTextArea();
			showNewJSONMaterialEditorButton();
			hideSaveMaterialDataButton();
			hideMaterialDataSaved();
...
function loaded() {
	openEventBridge(function () {
		...
		GROUPS.forEach(function (group) {
			...
			if (group.addToGroup !== undefined) {
				...
				elGroup = document.createElement("div");
				elGroup.className =
					"section " + (group.isMinor ? "minor" : "major");
			...
			group.properties.forEach(function (propertyData) {
				...
				let propertyName =
					propertyData.propertyName !== undefined
						? propertyData.propertyName
						: propertyID;
				let propertySpaceMode =
					propertyData.spaceMode !== undefined
						? propertyData.spaceMode
						: PROPERTY_SPACE_MODE.ALL;
				...
				if (propertyData.replaceID === undefined) {
					...
					let labelText =
						propertyData.label !== undefined
							? propertyData.label
							: "";
					let className = "";
					if (
						propertyData.indentedLabel ||
						propertyData.showPropertyRule !== undefined
					) {
						className = "indented";
					}
					...
					elContainer = document.getElementById(
						propertyData.replaceID
					);
				}

				if (elLabel) {
					createAppTooltip.registerTooltipElement(
						elLabel.childNodes[0],
						propertyID,
						propertyName
					);
				}
				...
				if (propertyType === "triple") {
					...
					for (let i = 0; i < propertyData.properties.length; ++i) {
						...
						let elWrapper = createElementFromHTML(
							'<div class="triple-item"></div>'
						);
						elProperty.appendChild(elWrapper);

						let propertyID = innerPropertyData.propertyID;
						let propertyName =
							innerPropertyData.propertyName !== undefined
								? innerPropertyData.propertyName
								: propertyID;
						let propertyElementID = "property-" + propertyID;
						propertyElementID = propertyElementID.replace(".", "-");
						...
						property.isParticleProperty = group.id.includes(
							"particles"
						);
						...
						let elLabel = createElementFromHTML(
							`<div class="triple-label">${innerPropertyData.label}</div>`
						);
						createAppTooltip.registerTooltipElement(
							elLabel,
							propertyID,
						...
						if (
							innerPropertyData.type === "number" ||
							innerPropertyData.type === "number-draggable"
						) {
					...
					if (
						...
						propertyData.type === "number-draggable" ||
						propertyData.type === "vec2" ||
						propertyData.type === "vec3" ||
						propertyData.type === "rect"
					) {
						propertyRangeRequests.push(propertyID);
					}
					...
					if (showPropertyRule !== undefined) {
						let dependentProperty = Object.keys(
							showPropertyRule
						)[0];
						let dependentPropertyValue =
							showPropertyRule[dependentProperty];
		...
		if (window.EventBridge !== undefined) {
			EventBridge.scriptEventReceived.connect(function (data) {
				...
				if (
					data.type === "server_script_status" &&
					...
					elServerScriptError.parentElement.style.display = data.errorInfo
						? "block"
						: "none";
					if (data.statusRetrieved === false) {
						elServerScriptStatus.innerText =
							"Failed to retrieve status";
					} else if (data.isRunning) {
						elServerScriptStatus.innerText =
							ENTITY_SCRIPT_STATUS[data.status] || data.status;
					} else {
						elServerScriptStatus.innerText = NOT_RUNNING_SCRIPT_STATUS;
					}
					...
					if (data.spaceMode !== undefined) {
						currentSpaceMode =
							data.spaceMode === "local"
								? PROPERTY_SPACE_MODE.LOCAL
								: PROPERTY_SPACE_MODE.WORLD;
					}
					handleEntitySelectionUpdate(
						data.selections,
						data.isPropertiesToolUpdate
					);
					...
					for (let property in propertyRanges) {
						...
						if (propertyRange !== undefined) {
							...
							if (
								propertyData.min === undefined &&
								propertyRange.minimum !== ""
							) {
							...
							if (
								propertyData.max === undefined &&
								propertyRange.maximum !== ""
							) {
			...
			EventBridge.emitWebEvent(
				JSON.stringify({ type: "tooltipsRequest" })
			);
			EventBridge.emitWebEvent(
				JSON.stringify({
					type: "propertyRangeRequest",
					properties: propertyRangeRequests
				})
			);
		}

		// Server Script Status
		let elServerScriptStatusOuter = document.getElementById(
			"div-property-serverScriptStatus"
		);
		let elServerScriptStatusContainer = document.getElementById(
			"div-property-serverScriptStatus"
		).childNodes[1];
		let serverScriptStatusElementID = "property-serverScripts-status";
		createAppTooltip.registerTooltipElement(
			elServerScriptStatusOuter.childNodes[0],
			"serverScriptsStatus"
		);
		...
		elUserDataEditorStatus.setAttribute(
			"id",
			userDataElementID + "-editorStatus"
		);
		...
		elDiv.childNodes[JSON_EDITOR_ROW_DIV_INDEX].appendChild(
			elUserDataSaved
		);
		...
		elStaticMaterialData.setAttribute(
			"id",
			materialDataElementID + "-static"
		);
		let elMaterialDataEditor = document.createElement("div");
		elMaterialDataEditor.setAttribute(
			"id",
			materialDataElementID + "-editor"
		);
		let elMaterialDataEditorStatus = document.createElement("div");
		elMaterialDataEditorStatus.setAttribute(
			"id",
			materialDataElementID + "-editorStatus"
		);
		let elMaterialDataSaved = document.createElement("span");
		elMaterialDataSaved.setAttribute(
			"id",
			materialDataElementID + "-saved"
		);
		...
		for (
			let collapseIndex = 0, numCollapsibles = elCollapsible.length;
			collapseIndex < numCollapsibles;
			++collapseIndex
		) {
			let curCollapsibleElement = elCollapsible[collapseIndex];
			curCollapsibleElement.addEventListener(
				"click",
		...
		for (
			let textAreaIndex = 0, numTextAreas = elTextareas.length;
			textAreaIndex < numTextAreas;
			++textAreaIndex
		) {
			let curTextAreaElement = elTextareas[textAreaIndex];
			setTextareaScrolling(curTextAreaElement);
			curTextAreaElement.addEventListener(
				"input",
		...
		for (
			let dropDownIndex = 0;
			dropDownIndex < elDropdowns.length;
			++dropDownIndex
		) {
			...
			for (
				...
				if (
					options[optionIndex].getAttribute("selected") === "selected"
				) {
			...
			for (
				let listOptionIndex = 0;
				listOptionIndex < options.length;
				++listOptionIndex
			) {
				let li = document.createElement("li");
				li.setAttribute("value", options[listOptionIndex].value);
				li.textContent =
					options[listOptionIndex].firstChild.textContent;
		...
		document.addEventListener(
			...
			function (keyUpEvent) {
				...
				if (
					elUserDataEditor.contains(keyUpEvent.target) ||
				...
				let controlKey = window.navigator.platform.startsWith("Mac")
					? metaKey
					: ctrlKey;
	...
	setTimeout(function () {
		EventBridge.emitWebEvent(
			JSON.stringify({ type: "propertiesPageReady" })
		);
	}, 1000);
}
