...
(function() {
	...
	function ScaleEntity(hand) {
		...
		this.otherHand = function() {
			return this.hand === dispatcherUtils.RIGHT_HAND
				? dispatcherUtils.LEFT_HAND
		...
		this.bumperPressed = function(controllerData) {
			return (
				...
				dispatcherUtils.BUMPER_ON_VALUE
			);
		};
		...
		this.getTargetProps = function(controllerData) {
			...
			for (var i = 0; i < nearbyEntityProperties.length; i++) {
				...
				var handPosition =
					controllerData.controllerLocations[this.hand].position;
				var distance = Vec3.distance(props.position, handPosition);
				if (
					distance >
					dispatcherUtils.NEAR_GRAB_RADIUS * sensorScaleFactor
				) {
					continue;
				}
				if (
					(dispatcherUtils.entityIsGrabbable(props) ||
						dispatcherUtils.propsArePhysical(props)) &&
					!props.locked
				) {
					return props;
				}
		...
		this.isReady = function(controllerData) {
			...
			if (
				...
				var otherHandTargetProps = otherModule.getTargetProps(
					controllerData
				);
				if (thisHandTargetProps && otherHandTargetProps) {
					if (thisHandTargetProps.id === otherHandTargetProps.id) {
						if (!entityIsGrabbable(thisHandTargetProps)) {
							return dispatcherUtils.makeRunningValues(
								false,
								[],
								[]
							);
						...
						this.scalingStartDistance = Vec3.length(
							Vec3.subtract(
								controllerData.controllerLocations[this.hand]
									.position,
								controllerData.controllerLocations[
									this.otherHand()
								].position
							)
						);
						this.scalingStartDimensions =
							thisHandTargetProps.dimensions;
