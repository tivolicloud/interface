	...
	function NearGrabEntity(hand) {
		...
		this.startGrab = function(targetProps) {
			...
			if (
				...
				grabData.grabFollowsController
			) {
				handJointIndex = getControllerJointIndex(this.hand);
			} else {
				handJointIndex = MyAvatar.getJointIndex(
					this.hand === RIGHT_HAND ? "RightHand" : "LeftHand"
				);
			}
			...
			var relativePosition = Entities.worldToLocalPosition(
				targetProps.position,
				MyAvatar.SELF_ID,
				handJointIndex
			);
			var relativeRotation = Entities.worldToLocalRotation(
				targetProps.rotation,
				MyAvatar.SELF_ID,
				handJointIndex
			);
			this.grabID = MyAvatar.grab(
				targetProps.id,
				handJointIndex,
				relativePosition,
				relativeRotation
		...
		this.getTargetProps = function(controllerData) {
			...
			for (var i = 0; i < nearbyEntityProperties.length; i++) {
				...
				var grabPosition =
					controllerData.controllerLocations[this.hand].position; // Is offset from hand position.
				var dist = distanceBetweenPointAndEntityBoundingBox(
					grabPosition,
					props
				);
				var distance = Vec3.distance(grabPosition, props.position);
				if (dist > nearGrabDistance || distance > nearGrabRadius) {
					// Only smallish entities can be near grabbed.
					continue;
				}
				if (entityIsGrabbable(props) || entityIsCloneable(props)) {
					if (!entityIsCloneable(props)) {
						...
						var groupRootProps = findGrabbableGroupParent(
							controllerData,
							props
						);
		...
		this.run = function(controllerData, deltaTime) {
			if (this.grabbing) {
				if (
					...
					controllerData.secondaryValues[this.hand] <
						TRIGGER_OFF_VALUE
				) {
				...
				var props =
					controllerData.nearbyEntityPropertiesByID[
						this.targetEntityID
					];
				if (!props) {
					props = Entities.getEntityProperties(
						this.targetEntityID,
				...
				if (
					...
					if (targetCloneable) {
						if (this.cloneAllowed) {
							...
							if (cloneID !== null) {
								var cloneProps = Entities.getEntityProperties(
									cloneID,
									DISPATCHER_PROPERTIES
								);
