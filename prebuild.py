...
def headSha():
    if shutil.which('git') is None:
        logger.warning("Unable to find git executable, can't caclulate commit ID")
        return '0xDEADBEEF'
    ...
    if not sha:
        ...
        logger.warning("Couldn't find git sha for repository {}".format(repo_dir))
    return sha
...
def parse_args():
    ...
    if True:
        ...
        args = parser.parse_args(['--android', 'questInterface', '--build-root', 'C:/git/tivoli/android/apps/questInterface/.externalNativeBuild/cmake/debug/arm64-v8a'])
    return args
