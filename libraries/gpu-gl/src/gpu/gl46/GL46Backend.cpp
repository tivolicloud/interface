...
Q_LOGGING_CATEGORY(gpugl46logging, "hifi.gpu.gl46")

using namespace gpu;
using namespace gpu::gl46;

GLint GL46Backend::MAX_COMBINED_SHADER_STORAGE_BLOCKS{ 0 };
GLint GL46Backend::MAX_UNIFORM_LOCATIONS{ 0 };

static void staticInit() {
    static std::once_flag once;
    std::call_once(once, [&] {
        glGetIntegerv(GL_MAX_COMBINED_SHADER_STORAGE_BLOCKS, &GL46Backend::MAX_COMBINED_SHADER_STORAGE_BLOCKS);
        glGetIntegerv(GL_MAX_UNIFORM_LOCATIONS, &GL46Backend::MAX_UNIFORM_LOCATIONS);
    });
}
const std::string GL46Backend::GL46_VERSION { "GL46" };

GL46Backend::GL46Backend(bool syncCache) : Parent(syncCache) {
    staticInit();
}

GL46Backend::GL46Backend() : Parent() {
    staticInit();
}

void GL46Backend::recycle() const {
    Parent::recycle();
}

void GL46Backend::draw(GLenum mode, uint32 numVertices, uint32 startVertex) {
    if (isStereo()) {
...
void GL46Backend::do_draw(const Batch& batch, size_t paramOffset) {
...
void GL46Backend::do_drawIndexed(const Batch& batch, size_t paramOffset) {
    Primitive primitiveType = (Primitive)batch._params[paramOffset + 2]._uint;
    GLenum mode = gl::PRIMITIVE_TO_GL[primitiveType];
...
void GL46Backend::do_drawInstanced(const Batch& batch, size_t paramOffset) {
...
void GL46Backend::do_drawIndexedInstanced(const Batch& batch, size_t paramOffset) {
    GLint numInstances = batch._params[paramOffset + 4]._uint;
...
void GL46Backend::do_multiDrawIndirect(const Batch& batch, size_t paramOffset) {
...
void GL46Backend::do_multiDrawIndexedIndirect(const Batch& batch, size_t paramOffset) {
    uint commandCount = batch._params[paramOffset + 0]._uint;
    GLenum mode = gl::PRIMITIVE_TO_GL[(Primitive)batch._params[paramOffset + 1]._uint];
