...
namespace gpu { namespace gl46 {
    ...
    class GL46Buffer : public GLBuffer {
    ...
    public:
        GL46Buffer(const std::weak_ptr<GLBackend>& backend, const Buffer& buffer, GLBuffer* original) : Parent(backend, buffer, allocate()) {
...
GLuint GL46Backend::getBufferID(const Buffer& buffer) {
    return GL46Buffer::getId<GL46Buffer>(*this, buffer);
}

GLuint GL46Backend::getBufferIDUnsynced(const Buffer& buffer) {
    return GL46Buffer::getIdUnsynced<GL46Buffer>(*this, buffer);
}

GLBuffer* GL46Backend::syncGPUObject(const Buffer& buffer) {
    return GL46Buffer::sync<GL46Buffer>(*this, buffer);
}


bool GL46Backend::bindResourceBuffer(uint32_t slot, const BufferPointer& buffer) {
...
void GL46Backend::releaseResourceBuffer(uint32_t slot) {
