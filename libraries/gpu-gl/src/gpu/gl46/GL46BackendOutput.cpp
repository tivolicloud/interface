...
class GL46Framebuffer : public gl::GLFramebuffer {
...
public:
    GL46Framebuffer(const std::weak_ptr<gl::GLBackend>& backend, const gpu::Framebuffer& framebuffer)
        : Parent(backend, framebuffer, allocate()) { }
};

gl::GLFramebuffer* GL46Backend::syncGPUObject(const Framebuffer& framebuffer) {
    return gl::GLFramebuffer::sync<GL46Framebuffer>(*this, framebuffer);
}

GLuint GL46Backend::getFramebufferID(const FramebufferPointer& framebuffer) {
    return framebuffer ? gl::GLFramebuffer::getId<GL46Framebuffer>(*this, *framebuffer) : 0;
}

void GL46Backend::do_blit(const Batch& batch, size_t paramOffset) {
