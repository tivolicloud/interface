...
//
#pragma once
#ifndef hifi_gpu_46_GL46Backend_h
#define hifi_gpu_46_GL46Backend_h
...
class GL46Backend : public GLBackend {
...
public:
    ...
    explicit GL46Backend(bool syncCache);
    GL46Backend();
    virtual ~GL46Backend() {
    ...
    static const std::string GL46_VERSION;
    const std::string& getVersion() const override { return GL46_VERSION; }

    bool supportedTextureFormat(const gpu::Element& format) override;

    class GL46Texture : public GLTexture {
        using Parent = GLTexture;
        friend class GL46Backend;
        static GLuint allocate(const Texture& texture);

    protected:
        GL46Texture(const std::weak_ptr<GLBackend>& backend, const Texture& texture);
...
#if GPU_BINDLESS_TEXTURES
    class GL46TextureTable : public GLObject<TextureTable> {
        static GLuint allocate();
        using Parent = GLObject<TextureTable>;

    public:
        using BindlessArray = std::array<GL46Texture::Bindless, TextureTable::COUNT>;

        GL46TextureTable(const std::weak_ptr<GLBackend>& backend, const TextureTable& texture);
        ~GL46TextureTable();
    ...
    class GL46FixedAllocationTexture : public GL46Texture {
        using Parent = GL46Texture;
        friend class GL46Backend;

    public:
        GL46FixedAllocationTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture);
        ~GL46FixedAllocationTexture();
    ...
    class GL46AttachmentTexture : public GL46FixedAllocationTexture {
        using Parent = GL46FixedAllocationTexture;
        friend class GL46Backend;

    protected:
        GL46AttachmentTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture);
        ~GL46AttachmentTexture();
    };

    class GL46StrictResourceTexture : public GL46FixedAllocationTexture {
        using Parent = GL46FixedAllocationTexture;
        friend class GL46Backend;

    protected:
        GL46StrictResourceTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture);
        ~GL46StrictResourceTexture();
    };
    ...
    class GL46VariableAllocationTexture : public GL46Texture, public GLVariableAllocationSupport {
        using Parent = GL46Texture;
        friend class GL46Backend;
        using PromoteLambda = std::function<void()>;

    protected:
        GL46VariableAllocationTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture);
        ~GL46VariableAllocationTexture();
    ...
    class GL46ResourceTexture : public GL46VariableAllocationTexture {
        using Parent = GL46VariableAllocationTexture;
        friend class GL46Backend;

    protected:
        GL46ResourceTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture);
...
#if 0
    class GL46SparseResourceTexture : public GL46VariableAllocationTexture {
        using Parent = GL46VariableAllocationTexture;
        friend class GL46Backend;
    ...
    protected:
        GL46SparseResourceTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture);
        ~GL46SparseResourceTexture();
...
#if GPU_BINDLESS_TEXTURES
    GL46TextureTable* syncGPUObject(const TextureTablePointer& textureTable);
    // Resource stage
...
Q_DECLARE_LOGGING_CATEGORY(gpugl46logging)
