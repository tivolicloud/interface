...
using GL46VariableAllocationTexture = GL46Backend::GL46VariableAllocationTexture;

GL46VariableAllocationTexture::GL46VariableAllocationTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture) : GL46Texture(backend, texture) {
}

GL46VariableAllocationTexture::~GL46VariableAllocationTexture() {
}

#if GPU_BINDLESS_TEXTURES
const GL46Texture::Bindless& GL46VariableAllocationTexture::getBindless() const {
...
Size GL46VariableAllocationTexture::copyMipFaceLinesFromTexture(uint16_t mip, uint8_t face, const uvec3& size, uint32_t yOffset, GLenum internalFormat, GLenum format, GLenum type, Size sourceSize, const void* sourcePointer) const {
    Size amountCopied = 0;
...
void copyTexGPUMem46(const gpu::Texture& texture, GLenum texTarget, GLuint srcId, GLuint destId, uint16_t numMips, uint16_t srcMipOffset, uint16_t destMipOffset, uint16_t populatedMips) {
...
void GL46VariableAllocationTexture::copyTextureMipsInGPUMem(GLuint srcId, GLuint destId, uint16_t srcMipOffset, uint16_t destMipOffset, uint16_t populatedMips) {
    uint16_t numMips = _gpuObject.getNumMips();
    copyTexGPUMem46(_gpuObject, _target, srcId, destId, numMips, srcMipOffset, destMipOffset, populatedMips);
}

// Managed size resource textures
using GL46ResourceTexture = GL46Backend::GL46ResourceTexture;

GL46ResourceTexture::GL46ResourceTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture) : GL46VariableAllocationTexture(backend, texture) {
...
void GL46ResourceTexture::allocateStorage(uint16 allocatedMip) {
...
Size GL46ResourceTexture::copyMipsFromTexture() {
    auto mipLevels = _gpuObject.getNumMips();
...
void GL46ResourceTexture::syncSampler() const {
    Parent::syncSampler();
#if GPU_BINDLESS_TEXTURES
    if (!isBindless()) {
...
size_t GL46ResourceTexture::promote() {
...
size_t GL46ResourceTexture::demote() {
    PROFILE_RANGE(render_gpu_gl, __FUNCTION__);
...
void GL46ResourceTexture::populateTransferQueue(TransferQueue& pendingTransfers) {
    PROFILE_RANGE(render_gpu_gl, __FUNCTION__);
    sanityCheck();
...
using GL46SparseResourceTexture = GL46Backend::GL46SparseResourceTexture;

GL46Texture::PageDimensionsMap GL46Texture::pageDimensionsByFormat;
Mutex GL46Texture::pageDimensionsMutex;

GL46Texture::PageDimensions GL46Texture::getPageDimensionsForFormat(const TextureTypeFormat& typeFormat) {
    {
...
GL46Texture::PageDimensions GL46Texture::getPageDimensionsForFormat(GLenum target, GLenum format) {
    return getPageDimensionsForFormat({ target, format });
}
bool GL46Texture::isSparseEligible(const Texture& texture) {
...
GL46SparseResourceTexture::GL46SparseResourceTexture(const std::weak_ptr<GLBackend>& backend, const Texture& texture) : GL46VariableAllocationTexture(backend, texture) {
    const GLTexelFormat texelFormat = GLTexelFormat::evalGLTexelFormat(_gpuObject.getTexelFormat());
    ...
    for (size_t i = 0; i < allowedPageDimensions.size(); ++i) {
        ...
        if (uvec3(0) == (dimensions % _pageDimensions)) {
            qCDebug(gpugl46logging) << "Enabling sparse for texture " << _gpuObject.source().c_str();
            break;
        }
    }
...
GL46SparseResourceTexture::~GL46SparseResourceTexture() {
    Backend::updateTextureGPUVirtualMemoryUsage(size(), 0);
}

uvec3 GL46SparseResourceTexture::getPageCounts(const uvec3& dimensions) const {
    auto result = (dimensions / _pageDimensions) +
        glm::clamp(dimensions % _pageDimensions, glm::uvec3(0), glm::uvec3(1));
    return result;
}

uint32_t GL46SparseResourceTexture::getPageCount(const uvec3& dimensions) const {
    auto pageCounts = getPageCounts(dimensions);
    return pageCounts.x * pageCounts.y * pageCounts.z;
}

void GL46SparseResourceTexture::promote() {
}

void GL46SparseResourceTexture::demote() {
}

SparseInfo::SparseInfo(GL46Texture& texture)
    : texture(texture) {
}
...
void SparseInfo::maybeMakeSparse() {
    ...
    for (size_t i = 0; i < allowedPageDimensions.size(); ++i) {
        ...
        if (uvec3(0) == (dimensions % pageDimensions)) {
            qCDebug(gpugl46logging) << "Enabling sparse for texture " << texture._source.c_str();
            sparse = true;
            break;
        }
    }
    ...
    if (sparse) {
        ...
        qCDebug(gpugl46logging) << "Size " << dimensions.x << " x " << dimensions.y <<
...
using SparseInfo = GL46Backend::GL46Texture::SparseInfo;

void GL46Texture::updateSize() const {
...
void GL46Texture::startTransfer() {
    Parent::startTransfer();
    _sparseInfo.update();
    _populatedMip = _maxMip + 1;
}

bool GL46Texture::continueTransfer() {
...
void GL46Texture::finishTransfer() {
    Parent::finishTransfer();
}

void GL46Texture::postTransfer() {
    Parent::postTransfer();
}

void GL46Texture::stripToMip(uint16_t newMinMip) {
    if (newMinMip < _minMip) {
        qCWarning(gpugl46logging) << "Cannot decrease the min mip";
        return;
    }

    if (_sparseInfo.sparse && newMinMip > _sparseInfo.maxSparseLevel) {
        qCWarning(gpugl46logging) << "Cannot increase the min mip into the mip tail";
        return;
    }

    // If we weren't generating mips before, we need to now that we're stripping down mip levels.
    if (!_gpuObject.isAutogenerateMips()) {
        qCDebug(gpugl46logging) << "Force mip generation for texture";
...
bool GL46Texture::derezable() const {
    if (_external) {
        return false;
    }
...
size_t GL46Texture::getMipByteCount(uint16_t mip) const {
    if (!_sparseInfo.sparse) {
...
std::pair<size_t, bool> GL46Texture::preDerez() {
...
size_t GL46Texture::getCurrentGpuSize() const {
    if (!_sparseInfo.sparse) {
...
size_t GL46Texture::getTargetGpuSize() const {
    if (!_sparseInfo.sparse) {
...
GL46Texture::~GL46Texture() {
    if (_sparseInfo.sparse) {
        ...
        if (0 != _sparseInfo.allocatedPages) {
            qCWarning(gpugl46logging) << "Allocated pages remaining " << _id << " " << _sparseInfo.allocatedPages;
        }
