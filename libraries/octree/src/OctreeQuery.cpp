...
int OctreeQuery::getBroadcastData(unsigned char* destinationBuffer) {
    ...
    if (!_jsonParameters.isEmpty()) {
        binaryParametersDocument = QJsonDocument(_jsonParameters).toJson(QJsonDocument::Compact);
    }
...
int OctreeQuery::parseData(ReceivedMessage& message) {
    ...
    if (binaryParametersBytes > 0) {
        ...
        auto newJsonValue = QJsonDocument::fromJson(binaryJSONParameters).toVariant().toJsonValue();
