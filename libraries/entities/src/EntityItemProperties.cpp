...
EntityPropertyFlags EntityItemProperties::getChangedProperties() const {
    ...
    CHECK_PROPERTY_CHANGE(PROP_LOCALLY_VISIBLE, locallyVisible);
    ...
    CHECK_PROPERTY_CHANGE(PROP_TRANSPARENT_BACKGROUND, transparentBackground);
 ...
 * @property {boolean} showKeyboardFocusHighlight=true - <code>true</code> if the entity is highlighted when it has keyboard focus, <code>false</code> if it isn't.
 * @property {boolean} transparentBackground=false - <code>true</code> if the web entity uses a transparent background, <code>false</code> if it doesnt.
...
QScriptValue EntityItemProperties::copyToScriptValue(QScriptEngine* engine, bool skipDefaults, bool allowUnknownCreateTime,
    ...
    COPY_PROPERTY_TO_QSCRIPTVALUE(PROP_LOCALLY_VISIBLE, locallyVisible);
    ...
    if (_type == EntityTypes::Web) {
        ...
        COPY_PROPERTY_TO_QSCRIPTVALUE(PROP_TRANSPARENT_BACKGROUND, transparentBackground);
    }
...
void EntityItemProperties::copyFromScriptValue(const QScriptValue& object, bool honorReadOnly) {
    ...
    COPY_PROPERTY_FROM_QSCRIPTVALUE(collisionless, bool, setCollisionless); // ok for statics
    COPY_PROPERTY_FROM_QSCRIPTVALUE_GETTER(ignoreForCollisions, bool, setCollisionless, getCollisionless); // ok for statics. legacy support
    COPY_PROPERTY_FROM_QSCRIPTVALUE(collisionMask, uint16_t, setCollisionMask); // ok for statics
    COPY_PROPERTY_FROM_QSCRIPTVALUE_ENUM(collidesWith, CollisionMask); // ok for statics
    ...
    COPY_PROPERTY_FROM_QSCRIPTVALUE(transparentBackground, bool, setTransparentBackground);
...
void EntityItemProperties::merge(const EntityItemProperties& other) {
    ...
    COPY_PROPERTY_IF_CHANGED(transparentBackground);
...
bool EntityItemProperties::getPropertyInfo(const QString& propertyName, EntityPropertyInfo& propertyInfo) {
    ...
    std::call_once(initMap, []() {
        ...
        ADD_PROPERTY_TO_MAP(PROP_LOCALLY_VISIBLE, LocallyVisible, locallyVisible, bool);
       ...
       // if (getEntityPriority() == EntityPriority::STATIC) return; // Not supported for static entities)
        ...
        ADD_PROPERTY_TO_MAP(PROP_TRANSPARENT_BACKGROUND, TransparentBackground, transparentBackground, bool);
    ...
    if (success) {
        ...
        if (headerFits) {
            ...
            if (properties.getType() == EntityTypes::Web) {
                ...
                APPEND_ENTITY_PROPERTY(PROP_TRANSPARENT_BACKGROUND, properties.getTransparentBackground());
            }
...
bool EntityItemProperties::decodeEntityEditPacket(const unsigned char* data, int bytesToRead, int& processedBytes,
    ...
    if (properties.getType() == EntityTypes::Web) {
        ...
        READ_ENTITY_PROPERTY_TO_PROPERTIES(PROP_TRANSPARENT_BACKGROUND, bool, setTransparentBackground);
    }
...
void EntityItemProperties::markAllChanged() {
    ...
    _transparentBackgroundChanged = true;
